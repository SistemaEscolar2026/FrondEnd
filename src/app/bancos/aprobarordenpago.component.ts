...
*/
@Component({
    templateUrl: './aprobarordenpago.component.html',
    styleUrls: ['./aprobarordenpago.component.scss']
})
export class AprobarOrdenPagoComponent implements OnInit {
    /**
 ...
 */
    ...
    cargaBancos() {
        this.spinner.show();
        this.bancosService.getOrdenPagoAprobar(globales.cia,"P").subscribe(data => {
            try {
   ...
   */
    ...
    confirmarguardar() {
        this.mensaje("4", "Esta seguro de realizar la aprobacion orden de pago");
  ...
  */
    insertdocumentobancos() {
        ...
        this.iDocumento.us_codigo_aprobacion = Funcion.returdatosession("usuario").us_codigo;
        this.iDocumento.bop_estado_aprobacion = "C";
        this.bancosService.updateOrdenAprobacion(this.iDocumento).subscribe(data => {
            try {
                this.spinner.hide();
                if (data.success) {
                    var mensaje: string = "";
  ...
  */
    ...
    mensaje(_tipo: string, _mensaje: string) {
        this.llamarmodal = _tipo + "|Aprobar Orden Pago|" + _mensaje + "|" + Funcion.Ramdon().toString();
    }
    /**
