...
*/
@Component({
    templateUrl: './mantenimientocuentabancaria.component.html',
    styleUrls: ['./mantenimientocuentabancaria.component.scss']
})
export class MantenimientoCuentaBancariaComponent implements OnInit {
    /**
  * DEFINICION DE VARIABLE DE MODAL DE BUSQUEDA DE VENDEDOR
  */
    @ViewChild('modalbusquedacuentabancaria') modalbusquedacuentabancaria: any;
    /**
...
*/
    cargaCuentaBancaria() {
        ...
        this.bancosService.manCuentaBancaria(globales.cia).subscribe(data => {
            try {
                this.listcuentabancaria = data.root[0];
                this.openModal(this.modalbusquedacuentabancaria);
                this.spinner.hide();
            } catch (e) {
                this.spinner.hide();
            }
  ...
  */
    deletecuentabancaria() {
        this.spinner.show();
        this.model.bcbcodigo = this.bancosfrom.controls['codigo'].value.toString().toUpperCase();
        this.model.bcbestado = "D";
        this.bancosService.deleteCuentaBancaria(this.model).subscribe(data => {
            try {
                if (data.success) {
  ...
  */
    insertbancos() {
        ...
        this.model.bcbcodigo = this.bancosfrom.controls['codigo'].value.toString().toUpperCase();
        this.model.bcbdescripcion = this.bancosfrom.controls['descripcion'].value.toString().toUpperCase();
        this.model.bcbestado = this.bancosfrom.controls['estado'].value.toString().toUpperCase();
        this.bancosService.insertCuentaBancaria(this.model).subscribe(data => {
            try {
                if (data.success) {
                    this.notifier.showok(data.msgError, "")
                    this.limpiabotones(1);
  ...
  */
    updatebancos() {
        ...
        this.model.bcbdescripcion = this.bancosfrom.controls['descripcion'].value.toString().toUpperCase();
        this.model.bcbestado = this.bancosfrom.controls['estado'].value.toString().toUpperCase();
        this.bancosService.updateCuentaBancaria(this.model).subscribe(data => {
            try {
                if (data.success) {
                    this.notifier.showok(data.msgError, "")
                    this.limpiabotones(1);
  ...
  */
    seteaCuentaBancaria(data: any) {

        this.bancosfrom.controls['codigo'].setValue(data.bcb_codigo);
        this.bancosfrom.controls['descripcion'].setValue(data.bcb_descripcion);
        this.bancosfrom.controls['estado'].setValue(data.bcb_estado);
  ...
  */
    cierreCuentaBancaria(event: any) {
        if (event !== "") {
            this.hideModal()
            this.seteaCuentaBancaria(JSON.parse(event))
        }
    }
    /**
  * DEFINICION DE FUNCION CIERRE MODAL DE VENDEDOR
