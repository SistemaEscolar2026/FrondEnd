...
*/
@Component({
    templateUrl: './mantenimientonumerocuenta.component.html',
    styleUrls: ['./mantenimientonumerocuenta.component.scss']
})
export class MantenimientoNumeroCuentaComponent implements OnInit {
    /**
  * DEFINICION DE VARIABLE DE MODAL DE BUSQUEDA DE VENDEDOR
  */
    @ViewChild('modalbusquedanumerocuenta') modalbusquedanumerocuenta: any;
    /**
...
*/
    cargaNumeroCuenta() {
        this.spinner.show();
        this.bancosService.manNumeroCuenta(globales.cia).subscribe(data => {
            try {
                this.listtipodocumento = data.root[0];
                this.openModal(this.modalbusquedanumerocuenta);
                this.spinner.hide();
            } catch (e) {
                this.spinner.hide();
            }
    ...
    habilitarfrom(val: Number) {
        switch (val) {
            case 1:
                ...
                this.bancosfrom.controls['banco'].disable();
                this.bancosfrom.controls['tipocuenta'].disable();
            ...
            case 2:
                ...
                this.bancosfrom.controls['banco'].enable();
                this.bancosfrom.controls['tipocuenta'].enable();
  ...
  */
    ...
    cargaTipoDocumento() {
        ...
        this.bancosService.manCuentaBancaria(globales.cia).subscribe(data => {
            try {
                this.listtipocuenta = data.root[0];
                this.spinner.hide();
            } catch (e) {
                this.spinner.hide();
            }
  ...
  */
    deletebancos() {
        this.spinner.show();
        this.model.bnccodigo = this.bancosfrom.controls['codigo'].value.toString().toUpperCase();
        this.model.bncestado = "D";
        this.bancosService.deleteNumeroCuenta(this.model).subscribe(data => {
            try {
                if (data.success) {
  ...
  */
    insertbancos() {
        ...
        this.model.bnccodigo = this.bancosfrom.controls['codigo'].value.toString().toUpperCase();
        this.model.bncnumero = this.bancosfrom.controls['descripcion'].value.toString().toUpperCase();
        this.model.bancodigo = this.bancosfrom.controls['banco'].value.toString().toUpperCase();
        this.model.bcbcodigo = this.bancosfrom.controls['tipocuenta'].value.toString().toUpperCase();
        this.model.bncestado = this.bancosfrom.controls['estado'].value.toString().toUpperCase();
        this.bancosService.insertNumeroCuenta(this.model).subscribe(data => {
            try {
                if (data.success) {
                    this.notifier.showok(data.msgError, "")
                    this.limpiabotones(1);
  ...
  */
    updatebancos() {
        ...
        this.model.bcbcodigo = this.bancosfrom.controls['tipocuenta'].value.toString().toUpperCase();
        this.model.bncestado = this.bancosfrom.controls['estado'].value.toString().toUpperCase();
        this.bancosService.updateNumeroCuenta(this.model).subscribe(data => {
            try {
                if (data.success) {
                    this.notifier.showok(data.msgError, "")
                    this.limpiabotones(1);
  ...
  */
    seteaPais(data: any) {
        ...
        this.bancosfrom.controls['codigo'].setValue(data.bnc_codigo);
        this.bancosfrom.controls['descripcion'].setValue(data.bnc_numero);
        this.bancosfrom.controls['banco'].setValue(data.ban_codigo);
        this.bancosfrom.controls['tipocuenta'].setValue(data.bcb_codigo);
        this.bancosfrom.controls['estado'].setValue(data.bnc_estado);
  ...
  */
    limpiarpais() {
        ...
        this.bancosfrom.controls['banco'].setValue("");
        this.bancosfrom.controls['tipocuenta'].setValue("");
