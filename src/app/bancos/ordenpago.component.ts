 ...
 */
    ...
    cargaBancos() {
        this.spinner.show();
        this.bancosService.getOrdenPago(globales.cia).subscribe(data => {
            try {
   ...
   */
    ...
    confirmarguardar() {
        if (this.iAccion === "I") {
            this.mensaje("4", "Esta seguro de guardar la orden pago");
        } else if (this.iAccion === "M") {
            this.mensaje("4", "Esta seguro de modificar la orden pago");
        }
  ...
  */
    insertdocumentobancos() {
        ...
        this.bancosService.insertOrdenPago(this.iDocumento).subscribe(data => {
            try {
                if (data.success) {
                    var mensaje: string = "";
  ...
  */
    ...
    buscardatos() {
        ...
        if (this.detalleregistrofact.length <= 0) {
            this.detalleregistrofact = [];
            this.spinner.show();
            this.boletoService.repoteCuentaxPagarSelec("", globales.cia, this.iDocumento.pr_codigo).subscribe(data => {
                try {
                    var datos: any[] = data.root[0];
                    datos.forEach((x) => {
                        if (x.selec === 1) {
                    ...
                    this.detalleregistrofact = datos;
    ...
    seteaDocumentoBancario(data: any) {
        ...
        this.bancosService.getOrdenPagoSele(globales.cia, data.bop_codigo).subscribe(data => {
            try {
                ...
                this.iDocumento.bop_fechaini = moment(this.iDocumento.bop_fechaini).format("YYYY-MM-DD").toString();
                this.iDocumento.bop_fechafin = moment(this.iDocumento.bop_fechafin).format("YYYY-MM-DD").toString();
  ...
  */
    ...
    mensaje(_tipo: string, _mensaje: string) {
        this.llamarmodal = _tipo + "|Registro Orden Pago|" + _mensaje + "|" + Funcion.Ramdon().toString();
    }
    /**
