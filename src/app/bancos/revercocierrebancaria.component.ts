...
*/
@Component({
    templateUrl: './revercocierrebancaria.component.html',
    styleUrls: ['./revercocierrebancaria.component.scss']
})
export class ReversoCierreBancariaComponent implements OnInit {
    /**
 ...
 */
   ...
   */
    ...
    confirmarguardar() {
        if (this.iAccion === "I") {
            this.mensaje("4", "Esta seguro de realizar el reverso del cierre de conciliacion bancario");
        }
  ...
  */
    ...
    cargaBancos() {
        this.spinner.show();
        this.bancosService.getCargaConciliacionCierreEstado(globales.cia).subscribe(data => {
            try {
  ...
  */
    insertdocumentobancos() {
        ...
        this.iDocumento.us_codigo_cierre = null;
        this.iDocumento.bco_cierre_estado = "P";
        this.iDocumento.bco_fechacierre =null;
        this.bancosService.updateCierreConcilacionBancaria(this.iDocumento).subscribe(data => {
            try {
                if (data.success) {
                    var mensaje: string = "";
  ...
  */
    ...
    buscardatos(bco_codigo: string) {
        this.iDocumento.bdo_debelibro = 0;
        this.iDocumento.bdo_haberlibro = 0;
        this.spinner.show();
        this.bancosService.cargacierreconciliacion(globales.cia, this.iDocumento.ban_codigo, this.iDocumento.bcb_codigo, this.iDocumento.bnc_codigo, this.iDocumento.bdo_fechaini, this.iDocumento.bdo_fechafin, bco_codigo).subscribe(data => {
            try {
                var datos: any[] = data.root[0];
                if (datos.length > 0) {
                    ...
                    this.iDocumento.bdo_fechacierre = moment(this.iDocumento.bdo_fechacierre).format("YYYY-MM-DD").toString();
  ...
  */
    ...
    mensaje(_tipo: string, _mensaje: string) {
        this.llamarmodal = _tipo + "|Reverso Cierre Conciliacion Bancario|" + _mensaje + "|" + Funcion.Ramdon().toString();
    }
    ...
    cierreConciliacionBancario(event: any) {
        if (event !== "") {
           ...
           // this.buscardatos2(data.bco_codigo);
            this.buscardatos(data.bco_codigo)
        }
    }
