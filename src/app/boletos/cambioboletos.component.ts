...
*/
@Component({
    templateUrl: './cambioboletos.component.html',
    styleUrls: ['./cambioboletos.component.scss']
})
export class CambioBoletosComponent implements OnInit {
    /**
 ...
 */
    ...
    confirmarguardar() {
        this.mensaje("4", "Esta seguro de guardar los cambios de valores");
    ...
    guardar() {
        ...
        model = this.detallereporte.filter((x) => x.cambio === true);

        this.boletoService.updateBoletoCambio(model).subscribe(data => {
            try {
                if (data.success) {
    ...
    buscarexcel() {
        this.spinner.show();
        let _lis = "";
        this.columnareporte.forEach((key: any) => {
            _lis = _lis + key.id + ",";
        });
        this.codreporte.forEach((key: any) => {
            if (key != "") {
                _lis = _lis.replace(key, '');
                //_lis = _lis + key + ",";
            }
        });
        this.boletoService.repoteBoletos("EXC", globales.cia, this.iDocumento.bc_fecha_inicio_viaje, this.iDocumento.bc_fecha_fin_viaje, this.iDocumento.cl_codigo.toString(), _lis, this.iDocumento.us_codigo.toString(), globales.nomMaquina, Funcion.ReturnUsuario().us_login).subscribe((results) => {
            try {
    ...
    buscarpdf() {
        ...
        this.boletoService.repoteBoletos("PDF", globales.cia, this.iDocumento.bc_fecha_inicio_viaje, this.iDocumento.bc_fecha_fin_viaje, this.iDocumento.cl_codigo.toString(), _lis, this.iDocumento.us_codigo.toString(), globales.nomMaquina, Funcion.ReturnUsuario().us_login).subscribe((results) => {
            try {
                try {
    ...
    buscarboleto() {
        ...
        this.codreporte.forEach((key: any) => {
            if (key != "") {
                if (key ==="IDENTIFICACION DE PASAJERO") {
                    this.col1 = true;
                }
                if (key === "RUC AEREOLINEA") {
                    this.col2 = true;
                }
                if (key === "PERSONA SOLICITA") {
                    this.col3 = true;
                }
                if (key === "SERVICIO EMISIÓN") {
                    this.col4 = true;
                }
                if (key === "IVA SERVICIO") {
                    this.col5 = true;
                }
                if (key === "TOTAL SERVICIOSO") {
                    this.col6 = true;
                }
            }
        });
        ...
        this.spinner.show();
        this.boletoService.repoteBoletosCambio("", globales.cia, this.iDocumento.bc_fecha_inicio_viaje, this.iDocumento.bc_fecha_fin_viaje, this.iDocumento.cl_codigo.toString(), this.iDocumento.bc_persona_solicita.toString()).subscribe(data => {
            try {
...
*/
  ...
  */
    ...
    mensaje(_tipo: string, _mensaje: string) {
        this.llamarmodal = _tipo + "|Autorizacion Cambio Valores Boleto|" + _mensaje + "|" + Funcion.Ramdon().toString();
    }
