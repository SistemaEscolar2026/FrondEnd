...
*/
@Component({
    templateUrl: './recuperaboletos.component.html',
    styleUrls: ['./recuperaboletos.component.scss']
})
export class RecuperaBoletosComponent implements OnInit {
    /**
 ...
 */
    ...
    activarboleto(row: any) {
        this.rowlinea = row;
        this.mensaje("4", "Esta seguro de Activar el Boleto");
    }
    ...
    procesar() {
        this.spinner.show();
        var datos: any = {};
        datos.bc_codigo = this.rowlinea.bc_codigo;
        datos.bc_estado_boleto = "1";
        this.boletoService.activarBoleto(datos).subscribe(data => {
            try {
                if (data.success) {
    ...
    buscarboleto() {
        this.spinner.show();
        this.boletoService.recuperaBoletos("", globales.cia, this.iDocumento.bc_fecha_inicio_viaje, this.iDocumento.bc_fecha_fin_viaje, this.iDocumento.cl_codigo.toString(), this.iDocumento.us_codigo.toString()).subscribe(data => {
            try {
  ...
  */
    constructor(private spinner: NgxSpinnerService, _router: Router, private modalService: BsModalService, private fb: FormBuilder, private mantenimientoService: MantenimientoService, private boletoService: BoletoService, private injector: Injector) {
        ...
        if (!Funcion.ValidadAutorizado()) {
            ...
            if (!Funcion.ValidadPagina('Recuperar Boleto')) {
...
*/
  ...
  */
    ...
    mensaje(_tipo: string, _mensaje: string) {
        this.llamarmodal = _tipo + "|Recuperar de Boleto|" + _mensaje + "|" + Funcion.Ramdon().toString();
    }
