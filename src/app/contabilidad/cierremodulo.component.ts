...
*/
@Component({
    selector: 'app-cierremodulo',
    templateUrl: './cierremodulo.component.html',
    styleUrls: ['./cierremodulo.component.scss']
})
export class CierreModuloComponent {
    ...
    procesar() {
        ...
        cierrermodulo.ccmfacturacion = (this.rb_facturacion) ? "C" : "A";
        cierrermodulo.ccmboleto = (this.rb_boleto) ? "C" : "A";
        cierrermodulo.ccmcuentacobrar = (this.rb_cxc) ? "C" : "A";
        cierrermodulo.ccmcuentapagar = (this.rb_cxp) ? "C" : "A";
        cierrermodulo.ccmordencompra = (this.rb_ordencompra) ? "C" : "A";
        cierrermodulo.ccmcontabilidad = (this.rb_contabilidad) ? "C" : "A";
        cierrermodulo.ccmnomina = (this.rb_nomina) ? "C" : "A";
        cierrermodulo.ccmbanco = (this.rb_banco) ? "C" : "A";
        this.spinner.show();
        this.boletoService.procesarCierre(cierrermodulo).subscribe(data => {
            try {
                if (data.success) {
                    this.modalmensaje("3", data.msgError);
                    sessionStorage.setItem("cierremodulo", Funcion.EncriptarAES(JSON.stringify(data.root[0])));
