...
*/
@Component({
    templateUrl: './reportecuentaxpagarret.component.html',
    styleUrls: ['./reportecuentaxpagarret.component.scss']
})
export class ReporteCuentaxPagarRetComponent implements OnInit {
    /**
  ...
  */
    ...
    modalRef: any;
    llamarmodal: string = "";
    ColumnaDetalle: any[] = ['N° FACTURA X PAGAR', 'PROVEEDOR', 'FECHA REGISTRO', 'TIPO REFERENCIA', 'NUMERO DE DOCUMENTO', 'ORDEN DE COMPRA', 'TIPO DE COMPRA',
        'SUBTOTAL', 'IVA', 'TOTAL', 'VALOR RETENCION FUENTE', 'VALOR RETENCION IVA', 'TOTAL - RETENCION'];
 ...
 */
    ...
    buscarexcel() {
        this.spinner.show();
        this.boletoService.repoteCuentaxPagarRet("EXC", globales.cia, this.iDocumento.bc_fecha_inicio_viaje, this.iDocumento.bc_fecha_fin_viaje, this.iDocumento.cl_codigo, globales.nomMaquina, Funcion.ReturnUsuario().us_login).subscribe((results) => {
            try {
                ...
                a.href = fileURL;
                a.download = "ReporteCuentaxPagar_Retencion.xlsx";
                a.click();
                this.spinner.hide();
    ...
    buscarpdf() {
        this.spinner.show();
        this.boletoService.repoteCuentaxPagarRet("PDF", globales.cia, this.iDocumento.bc_fecha_inicio_viaje, this.iDocumento.bc_fecha_fin_viaje, this.iDocumento.cl_codigo, globales.nomMaquina, Funcion.ReturnUsuario().us_login).subscribe((results) => {
            try {
                try {
    ...
    buscarcuentaxcobrar() {
        this.spinner.show();
        this.boletoService.repoteCuentaxPagarRetVisual("", globales.cia, this.iDocumento.bc_fecha_inicio_viaje, this.iDocumento.bc_fecha_fin_viaje, this.iDocumento.cl_codigo).subscribe(data => {
            try {
...
*/
    ...
    totalColumna(col: number) {
        ...
        if (col === 4) {
            this.detallereporte.forEach((key) => {
                _cantidad = _cantidad + key.valor_fuente;
            });
        }
        if (col === 5) {
            this.detallereporte.forEach((key) => {
                _cantidad = _cantidad + key.valor_iva;
            });
        }
        if (col === 6) {
            this.detallereporte.forEach((key) => {
                _cantidad = _cantidad + key.total_retencion;
            });
        }
  ...
  */
    constructor(private spinner: NgxSpinnerService, _router: Router, private modalService: BsModalService, private fb: FormBuilder, private mantenimientoService: MantenimientoService, private boletoService: BoletoService, private injector: Injector) {
        ...
        if (!Funcion.ValidadAutorizado()) {
            ...
            if (!Funcion.ValidadPagina('Reporte Cuenta x Cobrar')) {
    ...
    seteaCliente(_datos: any) {
        this.iDocumento.cl_codigo = _datos.pr_codigo;
...
*/
  ...
  */
    ...
    mensaje(_tipo: string, _mensaje: string) {
        this.llamarmodal = _tipo + "|Reporte de Cuenta x Cobrar|" + _mensaje + "|" + Funcion.Ramdon().toString();
    }
