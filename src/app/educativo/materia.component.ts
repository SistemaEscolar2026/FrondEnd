...
*/
    cargaNivel() {
        this.spinner.show();
        this.mantenimientoService.manMateria(globales.cia).subscribe(data => {
            try {
  ...
  */
    constructor(private spinner: NgxSpinnerService, _router: Router, private modalService: BsModalService, private fb: FormBuilder, private mantenimientoService: MantenimientoService, private injector: Injector) {
        ...
        if (!Funcion.ValidadAutorizado()) {
            ...
            if (!Funcion.ValidadPagina('Materia')) {
  ...
  */
    deletepais() {
        this.spinner.show();
        this.model.macodigo = this.paisfrom.controls['codigo'].value.toString().toUpperCase();
        this.model.maestado = "D";
        this.mantenimientoService.deleteMateria(this.model).subscribe(data => {
            try {
                if (data.success) {
  ...
  */
    insertpais() {
        this.spinner.show();
        this.model.macodigo = this.paisfrom.controls['codigo'].value.toString().toUpperCase();
        this.model.manombre = this.paisfrom.controls['descripcion'].value.toString().toUpperCase();
        this.model.maestado = this.paisfrom.controls['estado'].value.toString().toUpperCase();
        this.mantenimientoService.insertMateria(this.model).subscribe(data => {
            try {
                if (data.success) {
                    this.notifier.showok(data.msgError, "")
                    this.limpiabotones(1);
  ...
  */
    updatepais() {
        ...
        this.model.manombre = this.paisfrom.controls['descripcion'].value.toString().toUpperCase();
        this.model.maestado = this.paisfrom.controls['estado'].value.toString().toUpperCase();
        this.mantenimientoService.updateMateria(this.model).subscribe(data => {
            try {
                if (data.success) {
                    this.notifier.showok(data.msgError, "")
                    this.limpiabotones(1);
  ...
  */
    seteaNivel(data: any) {
        ...
        this.paisfrom.controls['codigo'].setValue(data.ma_codigo);
        this.paisfrom.controls['descripcion'].setValue(data.ma_nombre);
        this.paisfrom.controls['estado'].setValue(data.ma_estado);
