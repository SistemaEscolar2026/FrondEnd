...
export class NivelComponent implements OnInit {
  ...
  */
    @ViewChild('modalbusquedanivel') modalbusquedanivel: any;
    /**
...
*/
    cargaNivel() {
        ...
        this.mantenimientoService.manNivel(globales.cia).subscribe(data => {
            try {
                ...
                this.openModal(this.modalbusquedanivel);
                this.spinner.hide();
            } catch (e) {
                this.spinner.hide();
            }
  ...
  */
    constructor(private spinner: NgxSpinnerService, _router: Router, private modalService: BsModalService, private fb: FormBuilder, private mantenimientoService: MantenimientoService, private injector: Injector) {
        ...
        if (!Funcion.ValidadAutorizado()) {
            ...
            if (!Funcion.ValidadPagina('Nivel')) {
  ...
  */
    deletepais() {
        this.spinner.show();
        this.model.nacodigo = this.paisfrom.controls['codigo'].value.toString().toUpperCase();
        this.model.naestado = "D";
        this.mantenimientoService.deleteNivel(this.model).subscribe(data => {
            try {
                if (data.success) {
  ...
  */
    insertpais() {
        this.spinner.show();
        this.model.nacodigo = this.paisfrom.controls['codigo'].value.toString().toUpperCase();
        this.model.nanombre = this.paisfrom.controls['descripcion'].value.toString().toUpperCase();
        this.model.naestado = this.paisfrom.controls['estado'].value.toString().toUpperCase();
        this.mantenimientoService.insertNivel(this.model).subscribe(data => {
            try {
                if (data.success) {
                    this.notifier.showok(data.msgError, "")
                    this.limpiabotones(1);
  ...
  */
    updatepais() {
        ...
        this.model.nanombre = this.paisfrom.controls['descripcion'].value.toString().toUpperCase();
        this.model.naestado = this.paisfrom.controls['estado'].value.toString().toUpperCase();
        this.mantenimientoService.updateNivel(this.model).subscribe(data => {
            try {
                if (data.success) {
                    this.notifier.showok(data.msgError, "")
                    this.limpiabotones(1);
  ...
  */
    seteaNivel(data: any) {
        ...
        this.paisfrom.controls['codigo'].setValue(data.na_codigo);
        this.paisfrom.controls['descripcion'].setValue(data.na_nombre);
        this.paisfrom.controls['estado'].setValue(data.na_estado);
