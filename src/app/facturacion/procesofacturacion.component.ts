...
*/
@Component({
    templateUrl: './procesofacturacion.component.html',
    styleUrls: ['./procesofacturacion.component.scss']
})
export class ProcesoFacturacionComponent implements OnInit {
    /**
  ...
  */
    ...
    llamarmodal: string = "";
    alto: string = "";
    ColumnaDetalle: any[] = ['ENTIDAD', 'NUMERO FACTURA', 'FECHA DE FACTURA', 'SUBTOTAL', 'SUBTOTAL EXENTO', 'BASE IVA', 'IVA', 'TOTAL FACTURA', 'ESTADO DOCUMENTO', '', '', ''];
 ...
 */
    ...
    buscarexcel() {
        this.spinner.show();
        this.boletoService.repoteFacturacion("EXC", globales.cia, this.iDocumento.bc_fecha_inicio_viaje, this.iDocumento.bc_fecha_fin_viaje, this.iDocumento.cl_codigo.toString(), globales.nomMaquina, Funcion.ReturnUsuario().us_login).subscribe((results) => {
            try {
    ...
    buscarpdf() {
        this.spinner.show();
        this.boletoService.repoteFacturacion("PDF", globales.cia, this.iDocumento.bc_fecha_inicio_viaje, this.iDocumento.bc_fecha_fin_viaje, this.iDocumento.cl_codigo.toString(), globales.nomMaquina, Funcion.ReturnUsuario().us_login).subscribe((results) => {
            try {
                try {
    ...
    buscarfacturacion() {
        this.spinner.show();
        this.boletoService.repoteFacturacionVisual3("", globales.cia, this.iDocumento.bc_fecha_inicio_viaje, this.iDocumento.bc_fecha_fin_viaje, this.iDocumento.cl_codigo.toString()).subscribe(data => {
            try {
...
*/
  ...
  */
    ...
    generaRIDE(row: any) {
        ...
        this.spinner.show();
        this.boletoService.repoteFacturacion("PDFF", globales.cia, this.rowlinea.fa_codigo, "", "", globales.nomMaquina, Funcion.ReturnUsuario().us_login).subscribe((results) => {
            try {
                try {
                    var file = new Blob([results], { type: 'application/pdf' });
                    var fileURL = window.URL.createObjectURL(file);
    ...
    generaXML(row: any) {
        ...
        this.spinner.show();
        this.boletoService.repoteFacturacionXML("PDFF", globales.cia, this.rowlinea.fa_codigo, "", "", globales.nomMaquina, Funcion.ReturnUsuario().us_login).subscribe((results) => {
            try {
                try {
    ...
    procesar(row: any) {
        this.rowlinea = row;
        this.spinner.show();
        this.facturacionService.procesarFactura(this.rowlinea.fa_codigo).subscribe(data => {
            try {
                if (data.success) {
