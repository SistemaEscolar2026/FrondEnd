...
*/
@Component({
    templateUrl: './procesonotacredito.component.html',
    styleUrls: ['./procesonotacredito.component.scss']
})
export class ProcesoNotaCreditocomponent implements OnInit {
    /**
  ...
  */
    ...
    llamarmodal: string = "";
    alto: string = "";
    ColumnaDetalle: any[] = ['ENTIDAD', 'NUMERO NOTA CREDITO', 'FECHA DE NOTA CREDITO', 'SUBTOTAL', 'SUBTOTAL EXENTO', 'BASE IVA', 'IVA', 'TOTAL NOTA CREDITO', 'ESTADO DOCUMENTO', '', '', ''];
 ...
 */
    ...
    buscarexcel() {
        this.spinner.show();
        this.boletoService.repoteNotaCredito("EXC", globales.cia, this.iDocumento.bc_fecha_inicio_viaje, this.iDocumento.bc_fecha_fin_viaje, this.iDocumento.cl_codigo.toString(), globales.nomMaquina, Funcion.ReturnUsuario().us_login).subscribe((results) => {
            try {
                ...
                a.href = fileURL;
                a.download = "ReporteFacturacion.xlsx";
                a.click();
                this.spinner.hide();
    ...
    buscarpdf() {
        this.spinner.show();
        this.boletoService.repoteNotaCredito("PDF", globales.cia, this.iDocumento.bc_fecha_inicio_viaje, this.iDocumento.bc_fecha_fin_viaje, this.iDocumento.cl_codigo.toString(), globales.nomMaquina, Funcion.ReturnUsuario().us_login).subscribe((results) => {
            try {
                try {
    ...
    buscarfacturacion() {
        this.spinner.show();
        this.boletoService.repoteFacturacionVisualNC("", globales.cia, this.iDocumento.bc_fecha_inicio_viaje, this.iDocumento.bc_fecha_fin_viaje, this.iDocumento.cl_codigo.toString()).subscribe(data => {
            try {
...
*/
    ...
    totalColumna(col: number) {
        ...
        if (col === 1) {
            this.detallereporte.forEach((key) => {
                _cantidad = _cantidad + key.fa_subtotal;
            });
        }
        if (col === 2) {
            this.detallereporte.forEach((key) => {
                _cantidad = _cantidad + key.fa_subtotalexento;
            });
        }
        if (col === 3) {
            this.detallereporte.forEach((key) => {
                _cantidad = _cantidad + key.fa_baseiva;
            });
        }
        if (col === 4) {
            this.detallereporte.forEach((key) => {
                _cantidad = _cantidad + key.fa_iva;
            });
        }
        if (col === 5) {
            this.detallereporte.forEach((key) => {
        ...
        return parseFloat(_cantidad.toString()).toFixed(2);
    }
  ...
  */
    constructor(private spinner: NgxSpinnerService, _router: Router, private facturacionService: FacturacionService, private modalService: BsModalService, private fb: FormBuilder, private mantenimientoService: MantenimientoService, private boletoService: BoletoService, private injector: Injector) {
        this.router = _router;
        if (!Funcion.ValidadAutorizado()) {
            this.router.navigate(['/']);
        } else {
            if (!Funcion.ValidadPagina('Proceso Facturacion')) {
    ...
    confirmarguardar(row: any) {
        this.rowlinea = row;
        this.mensaje("4", "Esta seguro de procesar la factura");
    ...
    generaRIDE(row: any) {
        ...
        this.spinner.show();
        this.boletoService.repoteNotaCredito("PDFF", globales.cia, this.rowlinea.fa_codigo, "", "", globales.nomMaquina, Funcion.ReturnUsuario().us_login).subscribe((results) => {
            try {
                try {
                    var file = new Blob([results], { type: 'application/pdf' });
                    var fileURL = window.URL.createObjectURL(file);
    ...
    generaXML(row: any) {
        ...
        this.spinner.show();
        this.boletoService.repoteFacturacionXMLNC("PDFF", globales.cia, this.rowlinea.fa_codigo, "", "", globales.nomMaquina, Funcion.ReturnUsuario().us_login).subscribe((results) => {
            try {
                try {


                    var archivi: string = "FAC_" + row.num_factura.replace("-", "_") + ".xml";
                    var file = new Blob([(results)], { type: 'application/xml' });
                    var fileURL = window.URL.createObjectURL(file);
                    const a = document.createElement('a');
                    a.setAttribute('style', 'display:none;');
                    document.body.appendChild(a);
    ...
    aceptarConfi(event: boolean) {
        if (event) {
            //   this.procesarbuscarpdf
        }
    }
    procesar(row: any) {
        this.rowlinea = row;
        this.spinner.show();
        this.facturacionService.procesarNotaCredito(this.rowlinea.fa_codigo).subscribe(data => {
            try {
                if (data.success) {
    ...
    limpiarColor() {
        for (var i = 0; i < this.detallereporte.length; i++) {
            this.detallereporte[i].color = "";
        }
    }
    /**
...
*/
  ...
  */
    ...
    mensaje(_tipo: string, _mensaje: string) {
        this.llamarmodal = _tipo + "|Procesar Factura|" + _mensaje + "|" + Funcion.Ramdon().toString();
    }
