...
*/
@Component({
    templateUrl: './registrofacturacion.component.html',
    styleUrls: ['./registrofacturacion.component.scss']
})
export class RegistroFacturacionComponent implements OnInit {
    /**
  ...
  */
    ...
    ColumnaDetalle: any[] = ['', 'N°. BOLETO', 'CLIENTE', 'FECHA DE EMISION', 'RUTA', 'FECHA DE IDA', 'FECHA DE RETORNO', 'ABONO', 'TOTAL BOLETO + SERVICIO', 'SALDO', 'OPERADOR', '# DE TICKET', '# RESERVA'];
    /**
 ...
 */
    ...
    numerofactura() {
        this.spinner.show();
        this.facturacionService.numeroFactura(globales.cia).subscribe(data => {
            try {
                if (data.success) {
                    this.iDocumento.fa_numero_factura = data.root[0][0].factura;
                    this.generaclave();
                }
                this.spinner.hide();
            } catch (e) {
...
*/
    ...
    totalColumna(col: number) {
        ...
        this.iDocumento.fa_subtotalnoexento = 0;
        ...
        if (this.iDocumento.fa_tipofactura === 'B') {
            this.detalleregistrofactura.forEach((key) => {
                ...
                this.iDocumento.fa_subtotalnoexento = parseFloat(this.iDocumento.fa_subtotalnoexento) + parseFloat((key.fd_nobaseiva === "") ? 0 : key.fd_nobaseiva);
            ...
            this.detalleregistrofactura.forEach((key) => {
                ...
                this.iDocumento.fa_subtotalnoexento = parseFloat(this.iDocumento.fa_subtotalnoexento) + (parseFloat((key.fd_nobaseiva === "") ? 0 : key.fd_nobaseiva) * ((key.fd_cantidad === "") ? 1 : key.fd_cantidad));
    ...
    validar() {
        ...
        if (parseInt(this.iDocumento.fa_fecha_factura.substr(0, 4)) < parseInt(datos.ccm_anio)) {
            this.smensaje = "El periodo que desea ingresar ya esta cerrado ";
            this.submitted = true;
            return;
        }

        if (datos.ccm_facturacion === 'C') {
            this.smensaje = "El periodo que desea ingresar ya esta cerrado ";
            this.submitted = true;
            return;
        }
    ...
    modificarfactura() {
        if (this.iDocumento.fa_estado_electronico === 'RG') {
            ...
            this.mensaje("1", "No se puede modificar la factura ya esta en proceso del sri ...Por Favor Revisar")
            return;
        }
    }
    ...
    confirmareliminar() {
        if (this.iDocumento.fa_estado_electronico !== 'RG') {
            this.mensaje("1", "No se puede eliminar la factura ya esta en proceso del sri ...Por Favor Revisar")
            return;
        } else {
            this.mensaje("5", "Esta seguro de eliminar la factura N°." + this.iDocumento.fa_establecimiento + '-' + this.iDocumento.fa_punto_emision + '-' + this.iDocumento.fa_secuencial);
            return;
        }
    ...
    updateFactura() {
       ...
       // this.iDocumento.fa_clave = this.parentsvg.bcElement.nativeElement.innerHTML;
        ...
        this.facturacionService.updateFactura(this.iDocumento).subscribe(data => {
            try {
                if (data.success) {
...
/*        var img: any = document.getElementById('my-node')*/
        ...
        this.iDocumento.detalle = this.detalleregistrofactura;
        this.facturacionService.insertFactura(this.iDocumento).subscribe(data => {
            try {
                if (data.success) {
                    var mensaje: string = "";
                    mensaje = mensaje + data.msgError + "<br/>";
    ...
    generaclave() {
        ...
        var numero: string = (this.iDocumento.fa_numero_factura != "" && this.iDocumento.fa_numero_factura != undefined) ? this.iDocumento.fa_numero_factura.replace('-', '') : ""
        numero = numero.toString().replace("-", "");
        this.datoscla = fecha + "01" + compa.fae_ruc + compa.fae_tipo_ambiente + numero + "12345678" + compa.fae_tipo_emision;
  ...
  */
    ...
    buscarfactura() {
        this.spinner.show();
        this.boletoService.getFactura(globales.cia).subscribe(data => {
            try {
    ...
    calculoiva(row: any) {
        if (this.iDocumento.fa_tipofactura === 'B') {
            ...
            row.fd_nobaseiva = (row.fd_nobaseiva.toString() === "") ? '0' : row.fd_nobaseiva.toString();
            row.fd_subtotal = ((row.fd_baseiva.toString() === "") ? 0 : parseFloat(row.fd_baseiva.toString())) + ((row.fd_base0.toString() === "") ? 0 : parseFloat(row.fd_base0.toString())) + ((row.fd_nobaseiva.toString() === "") ? 0 : parseFloat(row.fd_nobaseiva.toString()));
