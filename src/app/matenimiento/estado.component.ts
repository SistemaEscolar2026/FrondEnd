...
export class EstadoComponent implements OnInit {
  ...
  */
    @ViewChild('modalbusquedaestado') modalbusquedaestado: any;
    /**
...
*/
    cargaEstado() {
        ...
        this.mantenimientoService.manEstado(globales.cia).subscribe(data => {
            try {
                ...
                this.openModal(this.modalbusquedaestado);
                this.spinner.hide();
            } catch (e) {
                this.spinner.hide();
            }
  ...
  */
    constructor(private spinner: NgxSpinnerService, _router: Router, private modalService: BsModalService, private fb: FormBuilder, private mantenimientoService: MantenimientoService, private injector: Injector) {
        ...
        if (!Funcion.ValidadAutorizado()) {
            ...
            if (!Funcion.ValidadPagina('Ingreso de Estado')) {
  ...
  */
    deleteestado() {
        ...
        this.model.escodigo = this.estadofrom.controls['codigo'].value.toString().toUpperCase();
        this.model.esestado = "D";
        this.mantenimientoService.deleteEstado(this.model).subscribe(data => {
            try {
                if (data.success) {
  ...
  */
    insertestado() {
        ...
        this.model.escodigo = this.estadofrom.controls['codigo'].value.toString().toUpperCase();
        this.model.esnombre = this.estadofrom.controls['descripcion'].value.toString().toUpperCase();
        this.model.esvalor = this.estadofrom.controls['valor'].value.toString().toUpperCase();
        this.model.esestado = this.estadofrom.controls['estado'].value.toString().toUpperCase();
        this.mantenimientoService.insertEstado(this.model).subscribe(data => {
            try {
                if (data.success) {
                    this.notifier.showok(data.msgError, "")
                    this.limpiabotones(1);
  ...
  */
    updateestado() {
        ...
        this.model.esvalor = this.estadofrom.controls['valor'].value.toString().toUpperCase();
        this.model.esestado = this.estadofrom.controls['estado'].value.toString().toUpperCase();
        this.mantenimientoService.updateEstado(this.model).subscribe(data => {
            try {
                if (data.success) {
                    this.notifier.showok(data.msgError, "")
                    this.limpiabotones(1);
  ...
  */
    seteaEstado(data: any) {
        ...
        this.estadofrom.controls['codigo'].setValue(data.es_codigo);
        this.estadofrom.controls['descripcion'].setValue(data.es_nombre);
        this.estadofrom.controls['valor'].setValue(data.es_valor);
        this.estadofrom.controls['estado'].setValue(data.es_estado);
