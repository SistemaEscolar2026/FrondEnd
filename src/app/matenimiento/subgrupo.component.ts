...
export class SubGrupoComponent implements OnInit {
  ...
  */
    @ViewChild('modalbusquedasubgrupo') modalbusquedasubgrupo: any;
    /**
...
*/
    cargaSubGrupo() {
        ...
        this.mantenimientoService.manSubGrupo(globales.cia).subscribe(data => {
            try {
                ...
                this.openModal(this.modalbusquedasubgrupo);
                this.spinner.hide();
            } catch (e) {
                this.spinner.hide();
            }
    ...
    habilitarfrom(val: Number) {
        switch (val) {
            case 1:
                ...
                this.parametrofrom.controls['coscodigo'].disable();
            ...
            case 2:
                ...
                this.parametrofrom.controls['concodigo'].enable();
  ...
  */
    constructor(private spinner: NgxSpinnerService, _router: Router, private modalService: BsModalService, private fb: FormBuilder, private mantenimientoService: MantenimientoService, private injector: Injector) {
        ...
        if (!Funcion.ValidadAutorizado()) {
            ...
            if (!Funcion.ValidadPagina('SubGrupo Contable')) {
  ...
  */
    ...
    deletesubgrupo() {
        this.spinner.show();
        this.model.coscodigo = this.parametrofrom.controls['coscodigo'].value.toString().toUpperCase();
        this.model.cocodigo = this.parametrofrom.controls['codigo'].value.toString().toUpperCase();
        this.model.cosestado = "D";
        this.mantenimientoService.deleteSubGrupo(this.model).subscribe(data => {
            try {
                if (data.success) {
  ...
  */
    ...
    insertsubgrupo() {
        ...
        this.model.cosdescripcion = this.parametrofrom.controls['descripcion'].value.toString().toUpperCase();
        this.model.coscodigo = this.parametrofrom.controls['coscodigo'].value.toString().toUpperCase();
        this.model.cosestado = this.parametrofrom.controls['estado'].value.toString().toUpperCase();
        this.model.uscodigo = Funcion.returdatosession("usuario").us_codigo;
        this.mantenimientoService.insertSubGrupo(this.model).subscribe(data => {
            try {
                if (data.success) {
                    this.notifier.showok(data.msgError, "")
                    this.limpiabotones(1);
  ...
  */
    updatesubgrupo() {
        ...
        this.model.cosestado = this.parametrofrom.controls['estado'].value.toString().toUpperCase();
        this.model.uscodigo = Funcion.returdatosession("usuario").us_codigo;
        this.mantenimientoService.UpdateSubGrupo(this.model).subscribe(data => {
            try {
                if (data.success) {
                    this.notifier.showok(data.msgError, "")
                    this.limpiabotones(1);
  ...
  */
    seteaParametro(data: any) {
        ...
        this.parametrofrom.controls['coscodigo'].setValue(data.cos_codigo);
        this.parametrofrom.controls['descripcion'].setValue(data.cos_descripcion);
        this.parametrofrom.controls['estado'].setValue(data.cos_estado);
  ...
  */
    limpiarparametro() {
        ...
        this.parametrofrom.controls['coscodigo'].setValue("");
