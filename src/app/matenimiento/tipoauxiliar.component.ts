...
*/
@Component({
    templateUrl: './tipoauxiliar.component.html',
    styleUrls: ['./tipoauxiliar.component.scss']
})
export class TipoAuxiliarComponent implements OnInit {
    /**
  * DEFINICION DE VARIABLE DE MODAL DE BUSQUEDA DE VENDEDOR
  */
    @ViewChild('modalbusquedatipoauxiliar') modalbusquedatipoauxiliar: any;
    /**
...
*/
    cargaGrupo() {
        ...
        this.mantenimientoService.manTipoAuxiliar(globales.cia).subscribe(data => {
            try {
                ...
                this.openModal(this.modalbusquedatipoauxiliar);
                this.spinner.hide();
            } catch (e) {
                this.spinner.hide();
            }
  ...
  */
    constructor(private spinner: NgxSpinnerService, _router: Router, private modalService: BsModalService, private fb: FormBuilder, private mantenimientoService: MantenimientoService, private injector: Injector) {
        ...
        if (!Funcion.ValidadAutorizado()) {
            ...
            if (!Funcion.ValidadPagina('Tipo Auxiliar')) {
  ...
  */
    deletetipoauxiliar() {
        this.spinner.show();
        this.model.ctacodigo = this.parametrofrom.controls['concodigo'].value.toString().toUpperCase();
        this.model.cocodigo = this.parametrofrom.controls['codigo'].value.toString().toUpperCase();
        this.model.ctaestado = "D";
        this.mantenimientoService.deleteTipoAuxiliar(this.model).subscribe(data => {
            try {
                if (data.success) {
  ...
  */
    insertgrupo() {
        ...
        this.model.ctacodigo = this.parametrofrom.controls['concodigo'].value.toString().toUpperCase();
        this.model.ctadescripcion = this.parametrofrom.controls['descripcion'].value.toString().toUpperCase();
        this.model.ctacodauxiliar = this.parametrofrom.controls['codauxiliar'].value.toString().toUpperCase();
        this.model.ctaestado = this.parametrofrom.controls['estado'].value.toString().toUpperCase();
        this.model.uscodigo = Funcion.returdatosession("usuario").us_codigo;
        this.mantenimientoService.insertTipoAuxiliar(this.model).subscribe(data => {
            try {
                if (data.success) {
                    this.notifier.showok(data.msgError, "")
                    this.limpiabotones(1);
  ...
  */
    updategrupo() {
        ...
        this.model.ctaestado = this.parametrofrom.controls['estado'].value.toString().toUpperCase();
        this.model.uscodigo = Funcion.returdatosession("usuario").us_codigo;
        this.mantenimientoService.UpdateTipoAuxiliar(this.model).subscribe(data => {
            try {
                if (data.success) {
                    this.notifier.showok(data.msgError, "")
                    this.limpiabotones(1);
  ...
  */
    seteaParametro(data: any) {
        this.parametrofrom.controls['concodigo'].setValue(data.cta_codigo);
        this.parametrofrom.controls['codigo'].setValue(data.co_codigo);
        this.parametrofrom.controls['codauxiliar'].setValue(data.cta_cod_auxiliar);
        this.parametrofrom.controls['descripcion'].setValue(data.cta_descripcion);
        this.parametrofrom.controls['estado'].setValue(data.cta_estado);
