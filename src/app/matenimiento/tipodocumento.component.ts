...
*/
  ...
  */
    constructor(private spinner: NgxSpinnerService, _router: Router, private modalService: BsModalService, private fb: FormBuilder, private mantenimientoService: MantenimientoService, private injector: Injector) {
        ...
        if (!Funcion.ValidadAutorizado()) {
            ...
            if (!Funcion.ValidadPagina('Ingreso de Tipo Documento')) {
  ...
  */
    deletetipodocumento() {
        ...
        this.model.tdcodigo = this.estadofrom.controls['codigo'].value.toString().toUpperCase();
        this.model.tdestado = "D";
        this.mantenimientoService.deleteTipoDocumento(this.model).subscribe(data => {
            try {
                if (data.success) {
  ...
  */
    insertetipodocumento() {
        ...
        this.model.tdcodigo = this.estadofrom.controls['codigo'].value.toString().toUpperCase();
        this.model.tdnombre = this.estadofrom.controls['descripcion'].value.toString().toUpperCase();
        this.model.tdvalor = this.estadofrom.controls['valor'].value.toString().toUpperCase();
        this.model.tdestado = this.estadofrom.controls['estado'].value.toString().toUpperCase();
        this.mantenimientoService.insertTipoDocumento(this.model).subscribe(data => {
            try {
                if (data.success) {
                    this.notifier.showok(data.msgError, "")
                    this.limpiabotones(1);
  ...
  */
    updatetipodocumento() {
        ...
        this.model.tdvalor = this.estadofrom.controls['valor'].value.toString().toUpperCase();
        this.model.tdestado = this.estadofrom.controls['estado'].value.toString().toUpperCase();
        this.mantenimientoService.updateTipoDocumento(this.model).subscribe(data => {
            try {
                if (data.success) {
                    this.notifier.showok(data.msgError, "")
                    this.limpiabotones(1);
  ...
  */
    seteaEstado(data: any) {
        ...
        this.estadofrom.controls['codigo'].setValue(data.td_codigo);
        this.estadofrom.controls['descripcion'].setValue(data.td_nombre);
        this.estadofrom.controls['valor'].setValue(data.td_valor);
        this.estadofrom.controls['estado'].setValue(data.td_estado);
