...
@Component({
    ...
    styleUrls: ['./tipoprestamo.component.scss']
})
...
*/
  ...
  */
    constructor(private spinner: NgxSpinnerService, _router: Router, private modalService: BsModalService, private fb: FormBuilder, private mantenimientoService: MantenimientoService, private injector: Injector) {
        ...
        if (!Funcion.ValidadAutorizado()) {
            ...
            if (!Funcion.ValidadPagina('Ingreso de Tipo Prestamo')) {
  ...
  */
    deletetipodocumento() {
        ...
        this.model.tpcodigo = this.estadofrom.controls['codigo'].value.toString().toUpperCase();
        this.model.tpestado = "D";
        this.mantenimientoService.deleteTipoPrestamo(this.model).subscribe(data => {
            try {
                if (data.success) {
  ...
  */
    insertetipodocumento() {
        this.spinner.show();
        this.model.cocodigo = globales.cia;
        this.model.tpcodigo = this.estadofrom.controls['codigo'].value.toString().toUpperCase();
        this.model.tpnombre = this.estadofrom.controls['descripcion'].value.toString().toUpperCase();
        this.model.tpvalor = this.estadofrom.controls['valor'].value.toString().toUpperCase();
        this.model.tpestado = this.estadofrom.controls['estado'].value.toString().toUpperCase();
        this.mantenimientoService.insertTipoPrestamo(this.model).subscribe(data => {
            try {
                if (data.success) {
                    this.notifier.showok(data.msgError, "")
                    this.limpiabotones(1);
  ...
  */
    updatetipodocumento() {
        ...
        this.model.tpvalor = this.estadofrom.controls['valor'].value.toString().toUpperCase();
        this.model.tpestado = this.estadofrom.controls['estado'].value.toString().toUpperCase();
        this.mantenimientoService.updateTipoPrestamo(this.model).subscribe(data => {
            try {
                if (data.success) {
                    this.notifier.showok(data.msgError, "")
                    this.limpiabotones(1);
  ...
  */
    seteaEstado(data: any) {
        ...
        this.estadofrom.controls['codigo'].setValue(data.tp_codigo);
        this.estadofrom.controls['descripcion'].setValue(data.tp_nombre);
        this.estadofrom.controls['valor'].setValue(data.tp_valor);
        this.estadofrom.controls['estado'].setValue(data.tp_estado);
