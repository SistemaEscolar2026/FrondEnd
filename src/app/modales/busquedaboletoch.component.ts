...
@Component({
    standalone: true,
    selector: 'app-busquedaboletoch',
    templateUrl: './busquedaboletoch.component.html',
    styleUrls: ['./busquedaboletoch.component.scss'],
    imports: [
        FormsModule,
        ReactiveFormsModule,
        MatButtonModule,
...
export class BusquedaBoletochComponent implements OnInit {
    /**
  ...
  */
    ColumnaDetalle: any[] = ['N° Boleto', 'Entidad', 'Nombre Pasajero', 'Ruta', 'Fecha inicio Viaje', 'Fecha Fin Viaje', 'Persona Solicita'];
    /**
  * DEFINICION DE VARIABLE CON DATOS DE CLIENTE
  */
    boletos: any[] = [];
    cambiogeneral: boolean = false;
    /**
  ...
  */
    ...
    cambio() {
        this.boletos.forEach((key) => {
            key.bd_select = this.cambiogeneral
        });
    }
    /**
  ...
  */
    aceptar() {
        ...
        let _codigo = "";
        this.boletos.forEach((k) => {
            if (k.bd_select) {
                _codigo = _codigo + k.bc_codigo + ",";
            }
        });
        this.selectedComision = _codigo;
        this.cerrar();
