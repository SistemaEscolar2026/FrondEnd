...
@Component({
    standalone: true,
    selector: 'app-busquedaclasecontable',
    templateUrl: './busquedaclasecontable.component.html',
    styleUrls: ['./busquedaclasecontable.component.scss'],
    imports: [
        FormsModule,
        ReactiveFormsModule,
        MatButtonModule,
...
export class BusquedaClaseContableComponent implements OnInit {
    /**
  * DEFINICION DE VARIABLE ENTRADA DE LISTADO DE CLIENTE
  */
    @Input() listclasecontable: any[] = [];
    /**
  ...
  */
    ngOnInit() {
        this.formaclasecontable = this.listclasecontable;
    }
    /**
  ...
  */
    limpiarColor() {
        for (var i = 0; i < this.formaclasecontable.length; i++) {
            this.formaclasecontable[i].color = "";
        }
    }
    /**
  ...
  */
    aceptar() {
        if (this.selectedComision === "") {
            this.toastr.info("Debe seleccionar una Clase Contable...", 'Busqueda Clase Contable');
        } else {
            this.cerrar();
        }
    }
