...
@Component({
    standalone: true,
    selector: 'app-busquedacuentabancaria',
    templateUrl: './busquedacuentabancaria.component.html',
    styleUrls: ['./busquedacuentabancaria.component.scss'],
    imports: [
        FormsModule,
        ReactiveFormsModule,
        MatButtonModule,
...
export class BusquedaCuentaBancariaComponent implements OnInit {
    /**
  * DEFINICION DE VARIABLE ENTRADA DE LISTADO DE CLIENTE
  */
    @Input() listcuentabancaria: any[] = [];
    /**
  ...
  */
    ngOnInit() {
        this.cuentabancaria = this.listcuentabancaria;
    }
    /**
  ...
  */
    limpiarColor() {
        for (var i = 0; i < this.cuentabancaria.length; i++) {
            this.cuentabancaria[i].color = "";
        }
    }
    /**
  ...
  */
    aceptar() {
        if (this.selectedComision === "") {
            this.toastr.info("Debe seleccionar una Cuenta Bancaria...", 'Busqueda Cuenta Bancaria');
        } else {
            this.cerrar();
        }
    }
