...
@Component({
    ...
    styleUrls: ['./busquedacurso.component.scss'],
    imports: [
        FormsModule,
        ReactiveFormsModule,
        MatButtonModule,
...
export class BusquedaCursoComponent implements OnInit {
  ...
  */
    ColumnaDetalle: any[] = ['Codigo', 'Descripcion', 'Nivel', 'Paralelo'];
    /**
  ...
  */
    ngOnInit() {
        this.formancurso = this.listadocurso;
    }
    /**
  ...
  */
    limpiarColor() {
        for (var i = 0; i < this.formancurso.length; i++) {
            this.formancurso[i].color = "";
        }
    }
    /**
  ...
  */
    aceptar() {
        if (this.selectedComision === "") {
            this.toastr.info("Debe seleccionar un Curso...", 'Busqueda Curso');
        } else {
            this.cerrar();
        }
    }
