...
@Component({
    standalone: true,
    selector: 'app-busquedadecimo',
    templateUrl: './busquedadecimo.component.html',
    styleUrls: ['./busquedadecimo.component.scss'],
    imports: [
        FormsModule,
        ReactiveFormsModule,
        MatButtonModule,
...
export class BusquedaDecimoComponent implements OnInit {
    /**
  * DEFINICION DE VARIABLE ENTRADA DE LISTADO DE CLIENTE
  */
    @Input() listdecimo: any[] = [];
    /**
  ...
  */
    ColumnaDetalle: any[] = ['N° Decimo', 'Año', 'Tipo','Observacion'];
    /**
  ...
  */
    limpiarColor() {
        for (var i = 0; i < this.decimos.length; i++) {
            this.decimos[i].color = "";
        }
    }
    /**
  ...
  */
    aceptar() {
        if (this.selectedComision === "") {
            this.toastr.info("Debe seleccionar un Decimos ...", 'Busqueda Decimos');
        } else {
            this.cerrar();
        }
    }
