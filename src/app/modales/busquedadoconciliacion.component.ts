...
@Component({
    standalone: true,
    selector: 'app-busquedadoconciliacion',
    templateUrl: './busquedadoconciliacion.component.html',
    styleUrls: ['./busquedadoconciliacion.component.scss'],
    imports: [
        FormsModule,
        ReactiveFormsModule,
        MatButtonModule,
...
export class BusquedaConciliacionComponent implements OnInit {
    /**
  * DEFINICION DE VARIABLE ENTRADA DE LISTADO DE CLIENTE
  */
    @Input() listconciliacion: any[] = [];
    /**
  ...
  */
    ColumnaDetalle: any[] = ['Codigo', 'Banco', 'Tipo Cuenta', 'Cuenta Bancaria', 'Conciliacion Desde', 'Conciliacion Hasta'];
    /**
  ...
  */
    ngOnInit() {
        this.conciliacion = this.listconciliacion;
    }
    /**
  ...
  */
    limpiarColor() {
        for (var i = 0; i < this.conciliacion.length; i++) {
            this.conciliacion[i].color = "";
        }
    }
    /**
  ...
  */
    aceptar() {
        if (this.selectedComision === "") {
            this.toastr.info("Debe seleccionar una conciliacion Bancario...", 'Busqueda Conciliacion Bancaria');
        } else {
            this.cerrar();
        }
    }
