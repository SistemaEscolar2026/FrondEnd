...
@Component({
    standalone: true,
    selector: 'app-busquedadocumentocxp',
    templateUrl: './busquedadocumentocxp.component.html',
    styleUrls: ['./busquedadocumentocxp.component.scss'],
    imports: [
        FormsModule,
        ReactiveFormsModule,
        MatButtonModule,
...
export class BusquedaAsientoContableComponent implements OnInit {
    /**
  ...
  */
    ColumnaDetalle: any[] = ['N° Docuemnto CXP', 'Fecha Documento', 'Glosa Documento'];
    /**
  ...
  */
    ngOnInit() {
        this.listaasiento = this.listcabecera;
    }
    /**
  ...
  */
    limpiarColor() {
        for (var i = 0; i < this.listaasiento.length; i++) {
            this.listaasiento[i].color = "";
        }
    }
    /**
  ...
  */
    aceptar() {
        if (this.selectedComision === "") {
            this.toastr.info("Debe seleccionar un Asiento Contable...", 'Busqueda Auxiliar');
        } else {
            this.cerrar();
        }
    }
