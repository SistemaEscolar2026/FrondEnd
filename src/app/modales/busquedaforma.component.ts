...
@Component({
    standalone: true,
    selector: 'app-busquedaforma',
    templateUrl: './busquedaforma.component.html',
    styleUrls: ['./busquedaforma.component.scss'],
    imports: [
        FormsModule,
        ReactiveFormsModule,
        MatButtonModule,
...
export class BusquedaFormaComponent implements OnInit {
    /**
  * DEFINICION DE VARIABLE ENTRADA DE LISTADO DE CLIENTE
  */
    @Input() listadoforma: any[] = [];
    /**
  ...
  */
    formapago: any[] = [];
    /**
* DEFINICION DE VARIABLE CON TIPO FORMA PAGO
*/
    listForma: any[] = [];
    /**
  ...
  */
    ngOnInit() {
        this.listForma = Funcion.TipoForma();
        this.formapago = this.listadoforma;
    }
    /**
  ...
  */
    limpiarColor() {
        for (var i = 0; i < this.formapago.length; i++) {
            this.formapago[i].color = "";
        }
    }
    /**
  ...
  */
    aceptar() {
        if (this.selectedComision === "") {
            this.toastr.info("Debe seleccionar una Comision...", 'Busqueda Cliente');
        } else {
            this.cerrar();
        }
    }
