...
@Component({
    standalone: true,
    selector: 'app-busquedanominames',
    templateUrl: './busquedanominames.component.html',
    styleUrls: ['./busquedanominames.component.scss'],
    imports: [
        FormsModule,
        ReactiveFormsModule,
        MatButtonModule,
...
export class BusquedaNominaMesComponent implements OnInit {
    /**
  * DEFINICION DE VARIABLE ENTRADA DE LISTADO DE CLIENTE
  */
    @Input() listfaltas: any[] = [];
    /**
  ...
  */
    ColumnaDetalle: any[] = ['N° Nomina', 'Año',  'Mes','Valor Nomina Fin Mes'];
    /**
  ...
  */
    ngOnInit() {
        this.formaaerolinea = this.listfaltas;
    }
    /**
  ...
  */
    aceptar() {
        if (this.selectedComision === "") {
            this.toastr.info("Debe seleccionar un NOmina Fin Mes...", 'Busqueda Nomina Fin Mes');
        } else {
            this.cerrar();
        }
    }
