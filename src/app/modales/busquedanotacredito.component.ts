...
@Component({
    standalone: true,
    selector: 'app-busquedanotacredito',
    templateUrl: './busquedanotacredito.component.html',
    styleUrls: ['./busquedanotacredito.component.scss'],
    imports: [
        FormsModule,
        ReactiveFormsModule,
        MatButtonModule,
...
export class BusquedaNotaCreditoComponent implements OnInit {
    /**
  ...
  */
    ColumnaDetalle: any[] = ['Nota Credito', 'Fecha Nota Credito', 'Entidad', 'Total'];
    /**
  ...
  */
    clickGrid(row: any) {
        this.limpiarColor();
        this.selectedComision = row.fa_codigo;
        row.color = "FT";
    }
    /**
  ...
  */
    aceptar() {
        if (this.selectedComision === "") {
            this.toastr.info("Debe seleccionar una Nota Credito...", 'Busqueda Nota Credito');
        } else {
            this.cerrar();
        }
    }
