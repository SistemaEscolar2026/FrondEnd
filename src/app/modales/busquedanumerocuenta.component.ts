...
@Component({
    standalone: true,
    selector: 'app-busquedanumerocuenta',
    templateUrl: './busquedanumerocuenta.component.html',
    styleUrls: ['./busquedanumerocuenta.component.scss'],
    imports: [
        FormsModule,
        ReactiveFormsModule,
        MatButtonModule,
...
export class BusquedaNumeroCuentaComponent implements OnInit {
    /**
  * DEFINICION DE VARIABLE ENTRADA DE LISTADO DE CLIENTE
  */
    @Input() listtipodocumento: any[] = [];
    /**
  ...
  */
    ColumnaDetalle: any[] = ['Codigo', 'Numero Cuenta'];
    /**
  ...
  */
    ngOnInit() {
        this.tipodocumento = this.listtipodocumento;
    }
    /**
  ...
  */
    limpiarColor() {
        for (var i = 0; i < this.tipodocumento.length; i++) {
            this.tipodocumento[i].color = "";
        }
    }
    /**
  ...
  */
    aceptar() {
        if (this.selectedComision === "") {
            this.toastr.info("Debe seleccionar un Numero Cuenta...", 'Busqueda Numero Cuenta');
        } else {
            this.cerrar();
        }
    }
