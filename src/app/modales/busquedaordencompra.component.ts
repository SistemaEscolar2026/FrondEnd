...
@Component({
    standalone: true,
    selector: 'app-busquedaordencompra',
    templateUrl: './busquedaordencompra.component.html',
    styleUrls: ['./busquedaordencompra.component.scss'],
    imports: [
        FormsModule,
        ReactiveFormsModule,
        MatButtonModule,
...
export class BusquedaOrdenCompraComponent implements OnInit {
    /**
  ...
  */
    ColumnaDetalle: any[] = ['N° Orden Compra', 'Fecha Orden Compra', 'Proveedor', 'Tipo Orden', 'Total'];
    /**
  ...
  */
    clickGrid(row: any) {
        this.limpiarColor();
        this.selectedComision = row.ord_codigo;
        row.color = "FT";
    }
    /**
  ...
  */
    aceptar() {
        if (this.selectedComision === "") {
            this.toastr.info("Debe seleccionar una Orden Compra...", 'Busqueda Orden Compra');
        } else {
            this.cerrar();
        }
    }
