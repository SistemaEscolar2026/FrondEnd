...
@Component({
    standalone: true,
    selector: 'app-busquedaparalelo',
    templateUrl: './busquedaparalelo.component.html',
    styleUrls: ['./busquedaparalelo.component.scss'],
    imports: [
        FormsModule,
        ReactiveFormsModule,
        MatButtonModule,
...
export class BusquedaParaleloComponent implements OnInit {
    /**
  * DEFINICION DE VARIABLE ENTRADA DE LISTADO DE CLIENTE
  */
    @Input() listadoparalelo: any[] = [];
    /**
  ...
  */
    ngOnInit() {
        this.formanparalelo = this.listadoparalelo;
    }
    /**
  ...
  */
    limpiarColor() {
        for (var i = 0; i < this.formanparalelo.length; i++) {
            this.formanparalelo[i].color = "";
        }
    }
    /**
  ...
  */
    aceptar() {
        if (this.selectedComision === "") {
            this.toastr.info("Debe seleccionar un Paralelo...", 'Busqueda Paralelo');
        } else {
            this.cerrar();
        }
    }
