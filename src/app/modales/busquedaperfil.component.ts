...
@Component({
    standalone: true,
    selector: 'app-busquedaperfil',
    templateUrl: './busquedaperfil.component.html',
    styleUrls: ['./busquedaperfil.component.scss'],
    imports: [
        FormsModule,
        ReactiveFormsModule,
        MatButtonModule,
...
export class BusquedaPerfilComponent implements OnInit {
    /**
  * DEFINICION DE VARIABLE ENTRADA DE LISTADO DE CLIENTE
  */
    @Input() listadoperfil: any[] = [];
    /**
  ...
  */
    ngOnInit() {
        this.formaperfil = this.listadoperfil;
    }
    /**
  ...
  */
    limpiarColor() {
        for (var i = 0; i < this.formaperfil.length; i++) {
            this.formaperfil[i].color = "";
        }
    }
    /**
  ...
  */
    aceptar() {
        if (this.selectedComision === "") {
            this.toastr.info("Debe seleccionar un Perfil...", 'Busqueda Perfil');
        } else {
            this.cerrar();
        }
    }
