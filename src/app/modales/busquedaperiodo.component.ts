...
@Component({
    standalone: true,
    selector: 'app-busquedaperiodo',
    templateUrl: './busquedaperiodo.component.html',
    styleUrls: ['./busquedaperiodo.component.scss'],
    imports: [
        FormsModule,
        ReactiveFormsModule,
        MatButtonModule,
...
export class BusquedaPerioroComponent implements OnInit {
    /**
  * DEFINICION DE VARIABLE ENTRADA DE LISTADO DE CLIENTE
  */
    @Input() listadomateria: any[] = [];
    /**
  ...
  */
    ColumnaDetalle: any[] = ['Codigo', 'Descripcion','Periodo'];
    /**
  ...
  */
    ngOnInit() {
        this.formamateria = this.listadomateria;
    }
    /**
  ...
  */
    limpiarColor() {
        for (var i = 0; i < this.formamateria.length; i++) {
            this.formamateria[i].color = "";
        }
    }
    /**
  ...
  */
    aceptar() {
        if (this.selectedComision === "") {
            this.toastr.info("Debe seleccionar un Periodo...", 'Busqueda Periodo');
        } else {
            this.cerrar();
        }
    }
