...
@Component({
    standalone: true,
    selector: 'app-busquedapersona',
    templateUrl: './busquedapersona.component.html',
    styleUrls: ['./busquedapersona.component.scss'],
    imports: [
        FormsModule,
        ReactiveFormsModule,
        MatButtonModule,
...
export class BusquedaPersonaAutorizadaComponent implements OnInit {
    /**
  * DEFINICION DE VARIABLE ENTRADA DE LISTADO DE CLIENTE
  */
    @Input() listapersona: any[] = [];
    /**
  ...
  */
    ColumnaDetalle: any[] = ['Valor', 'Descripcion'];
    /**
  * DEFINICION DE VARIABLE CON DATOS DE CLIENTE
  */
    formapersonalinea: any[] = [];
    /**
  ...
  */
    ngOnInit() {
        this.formapersonalinea = this.listapersona;
    }
    /**
  ...
  */
    limpiarColor() {
        for (var i = 0; i < this.formapersonalinea.length; i++) {
            this.formapersonalinea[i].color = "";
        }
    }
    /**
  ...
  */
    aceptar() {
        if (this.selectedComision === "") {
            this.toastr.info("Debe seleccionar un Persona Autorizada...", 'Busqueda Persona Autorizada');
        } else {
            this.cerrar();
        }
    }
