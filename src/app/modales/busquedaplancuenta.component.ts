...
@Component({
    standalone: true,
    selector: 'app-busquedaplancuenta',
    templateUrl: './busquedaplancuenta.component.html',
    styleUrls: ['./busquedaplancuenta.component.scss'],
    imports: [
        FormsModule,
        ReactiveFormsModule,
        MatButtonModule,
...
export class BusquedaPlanCuentaComponent implements OnInit {
    /**
  * DEFINICION DE VARIABLE ENTRADA DE LISTADO DE CLIENTE
  */
    @Input() listcuentas: any[] = [];
    /**
  ...
  */
    ColumnaDetalle: any[] = ['Codigo','Cuenta', 'Descripcion'];
    /**
  ...
  */
    ngOnInit() {
        this.formagrupo = this.listcuentas;
    }
    /**
  ...
  */
    limpiarColor() {
        for (var i = 0; i < this.formagrupo.length; i++) {
            this.formagrupo[i].color = "";
        }
    }
    /**
  ...
  */
    aceptar() {
        if (this.selectedComision === "") {
            this.toastr.info("Debe seleccionar un Grupo Contable...", 'Busqueda Grupo Contable');
        } else {
            this.cerrar();
        }
    }
