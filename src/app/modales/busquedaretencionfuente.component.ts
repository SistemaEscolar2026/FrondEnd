...
@Component({
    standalone: true,
    selector: 'app-modalbusquedaretencionfuente',
    templateUrl: './busquedaretencionfuente.component.html',
    styleUrls: ['./busquedaretencionfuente.component.scss'],
    imports: [
        FormsModule,
        ReactiveFormsModule,
        MatButtonModule,
...
export class BusquedaRetencionFuenteComponent implements OnInit {
    /**
  ...
  */
    ColumnaDetalle: any[] = ['Codigo', 'Descripcion','Porcentaje'];
    /**
  ...
  */
    ngOnInit() {
        this.formaretencion = this.listretencion;
    }
    /**
  * DEFINICION DE FUNCION DE CARGA DE COMISION
  */
    cargaFormaRetenciones() {
  ...
  */
    limpiarColor() {
        for (var i = 0; i < this.formaretencion.length; i++) {
            this.formaretencion[i].color = "";
        }
    }
    /**
  ...
  */
    aceptar() {
        if (this.selectedComision === "") {
            this.toastr.info("Debe seleccionar un Estado...", 'Busqueda Estado');
        } else {
            this.cerrar();
        }
    }
