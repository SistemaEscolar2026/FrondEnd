...
@Component({
    standalone: true,
    selector: 'app-busquedaservicio',
    templateUrl: './busquedaservicio.component.html',
    styleUrls: ['./busquedaservicio.component.scss'],
    imports: [
        FormsModule,
        ReactiveFormsModule,
        MatButtonModule,
...
*/
  ...
  */
    ngOnInit() {
        ...
        this.formaservicio = this.listadoservicio;
    }
    /**
  ...
  */
    limpiarColor() {
        for (var i = 0; i < this.formaservicio.length; i++) {
            this.formaservicio[i].color = "";
        }
    }
    /**
  ...
  */
    aceptar() {
        if (this.selectedComision === "") {
            this.toastr.info("Debe seleccionar un servicio Adicional...", 'Busqueda Servicio Adicional');
        } else {
            this.cerrar();
        }
    }
