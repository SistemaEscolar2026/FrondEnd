...
@Component({
    standalone: true,
    selector: 'app-busquedatipoprestamo',
    templateUrl: './busquedatipoprestamo.component.html',
    styleUrls: ['./busquedatipoprestamo.component.scss'],
    imports: [
        FormsModule,
        ReactiveFormsModule,
        MatButtonModule,
...
export class BusquedaTipoPrestamoComponent implements OnInit {
    /**
  ...
  */
    ngOnInit() {
        this.formaestado = this.listestados;
    }
    /**
  ...
  */
    limpiarColor() {
        for (var i = 0; i < this.formaestado.length; i++) {
            this.formaestado[i].color = "";
        }
    }
    /**
  ...
  */
    aceptar() {
        if (this.selectedComision === "") {
            this.toastr.info("Debe seleccionar un Tipo Prestamo...", 'Busqueda Tipo Prestamo');
        } else {
            this.cerrar();
        }
    }
