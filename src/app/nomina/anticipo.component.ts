...
*/
    cargaAerolinea() {
        this.spinner.show();
        this.mantenimientoService.getAnticipo(globales.cia).subscribe(data => {
            try {
                this.listfaltas = data.root[0];
  ...
  */
    ...
    delete() {
        this.spinner.show();
        this.iFalta.nat_estado = "D";
        this.mantenimientoService.deleteAnticipo(this.iFalta).subscribe(data => {
            try {
                if (data.success) {
  ...
  */
    ...
    updateFalta() {
        this.spinner.show();
        this.mantenimientoService.updateAnticipo(this.iFalta).subscribe(data => {
            try {
                if (data.success) {
                    var mensaje: string = "";
    ...
    insertAnticipo() {
        this.spinner.show();
        this.iFalta.nat_estado = "A";
        this.iFalta.nat_estadoanticipo = "A";
        this.iFalta.us_codigo = Funcion.returdatosession("usuario").us_codigo;
        this.mantenimientoService.insertAnticipo(this.iFalta).subscribe(data => {
            try {
                if (data.success) {
                    var mensaje: string = "";
                    mensaje = mensaje + data.msgError + "<br/>";
    ...
    validar() {
        ...
        if (this.iFalta.nat_valoranticipo === "" || this.iFalta.nat_valoranticipo === undefined || this.iFalta.nat_valoranticipo === "0") {
            this.smensaje = "No a Ingresado el valor del anticipo...Por Favor Revisar"
            return;
        }


        if (this.iFalta.nat_observacion === "") {
            this.smensaje = "No a Ingresado una observacion del anticipo...Por Favor Revisar"
            return;
        }

        var valor: number = (parseFloat(this.iFalta.empsueldo) * 30) / 100;

        if (parseFloat(this.iFalta.nat_valoranticipo)>valor) {
            this.smensaje = "El anticipo no puedo superar el 30% del sueldo ...Por Favor Revisar"
            return;
        }
  ...
  */
    seteaParametro(data: any) {
        ...
        this.iFalta.nat_fechaanticipo = moment(data.nat_fechaanticipo).format("YYYY-MM-DD").toString();
  ...
  */
    limpiarparametro() {
        ...
        this.iFalta.nat_fechaanticipo = moment(new Date()).format("YYYY-MM-DD").toString()
        this.iFalta.nat_estado = "A";
        this.iFalta.nat_observacion = "";
    }
  ...
  */
    ...
    mensaje(_tipo: string, _mensaje: string) {
        this.llamarmodal = _tipo + "|Registro Anticipo|" + _mensaje + "|" + Funcion.Ramdon().toString();
    }
}
