...
*/
@Component({
    templateUrl: './nominafinmes.component.html',
    styleUrls: ['./nominafinmes.component.scss']
})
export class NominaFinMesComponent implements OnInit {
    /**
  * DEFINICION DE VARIABLE DE MODAL DE BUSQUEDA DE VENDEDOR
  */
    @ViewChild('modalbusquedanominames') modalbusquedanominames: any;
  ...
  */
   ...
   */
    router: Router;
    ban: number = 0;
    ColumnaDetalle: any[] = ['Identificacion', 'Nombre Empleado', 'Sueldo Empleado', 'Adelanto Quincena', 'IESS', 'Decimo Tercero', 'Decimo Cuarto', 'Anticipo Quincena', 'Prestamo', 'Fondo Reserva', 'Total Recibir'];
    /**
...
*/
    cargaAnticipo() {
        this.spinner.show();
        this.mantenimientoService.getNomianFinMes(globales.cia).subscribe(data => {
            try {
                this.listanticipoquincena = data.root[0];
                this.openModal(this.modalbusquedanominames);
                this.spinner.hide();
            } catch (e) {
                this.spinner.hide();
            }
    ...
    limpiarColor() {
        for (var i = 0; i < this.detallenominafinmes.length; i++) {
            this.detallenominafinmes[i].color = "";
        }
    }
    /**
...
*/
    ...
    calcularcuota() {
        this.detallenominafinmes = [];
        this.spinner.show();
        this.mantenimientoService.generafinmes(globales.cia, this.iDocumento.nfm_anio, this.iDocumento.nfm_mes).subscribe(data => {
            try {
                if (data.codError === "0") {
                    this.detallenominafinmes = data.root[0];
                    this.detallefinmes = data.root[1];
                } else {
    ...
    agregarlinea() {
        var linea: any = {};
        linea.linea = this.detallenominafinmes.length + 1;
        this.detallenominafinmes.push(linea);
  ...
  */
    borrarlinea() {
        for (var i = 0; i < this.detallenominafinmes.length; i++) {
            if (this.detallenominafinmes[i].npd_linea === this.selectedlineagrid) {
                this.detallenominafinmes.splice(i, 1);
                this.selectedlineagrid = "";
                break;
            }
        }
  ...
  */
    ...
    delete() {
        this.spinner.show();
        this.iDocumento.nfm_estado = "D";
        this.mantenimientoService.deletFinMes(this.iDocumento).subscribe(data => {
            try {
                if (data.success) {
  ...
  */
    ...
    confirmarguardar() {
        if (this.isAccion === "I") {
            this.mensaje("4", "Esta seguro de generar nomina de fin de mes");
        } else if (this.isAccion === "M") {
            this.mensaje("4", "Esta seguro de modificar nomina de fin de mes");
        }

    }
    updatePrestamo() {
        this.spinner.show();
        this.iDocumento.detalle = this.detallefinmes;
    ...
    calculoquincena() {
        this.iDocumento.nfm_valototal = "";
        this.detallenominafinmes.forEach((x) => {
            this.iDocumento.nfm_valototal = ((this.iDocumento.nfm_valototal === "") ? 0 : parseFloat(this.iDocumento.nfm_valototal)) + parseFloat(x.total);
        });
        return this.iDocumento.nfm_valototal;
    }

    insertPrestamo() {
        this.spinner.show();
        this.iDocumento.co_codigo = globales.cia;
        this.iDocumento.nfm_estado = "A";
        this.iDocumento.us_codigo = Funcion.returdatosession("usuario").us_codigo;
        this.iDocumento.detalle = this.detallefinmes
        this.mantenimientoService.insertNominaFinMes(this.iDocumento).subscribe(data => {
            try {
                if (data.success) {
                    var mensaje: string = "";
                    mensaje = mensaje + data.msgError + "<br/>";
    ...
    validar() {
        ...
        if (this.detallenominafinmes.length <= 0) {
    ...
    seteaParametro(data: any) {
        ...
        this.spinner.show();
        this.mantenimientoService.getSeleNominaFinMes(data.co_codigo, data.nfm_codigo).subscribe(data => {
            try {
                if (data.codError === "0") {
                    this.iDocumento = data.root[0][0];
                    this.iDocumento.nfm_fecharegistro = moment(data.nfm_fecharegistro).format("YYYY-MM-DD").toString();

                    this.iDocumento.nfm_mes = parseInt(this.iDocumento.nfm_mes);
                    this.iDocumento.nfm_anio = parseInt(this.iDocumento.nfm_anio);

                    this.detallenominafinmes = data.root[1];
                    this.detallefinmes = data.root[2];
                    this.limpiabotones(3);
                } else {
                    this.mensaje("1", data.msgError);
                }
  ...
  */
    limpiarparametro() {
        this.iDocumento = {};
        this.iDocumento.nfm_fecharegistro = moment(new Date()).format("YYYY-MM-DD").toString()
        this.iDocumento.nfm_mes = parseInt( moment(new Date()).format("YYYY-MM-DD").toString().substr(5, 2));
        this.iDocumento.nfm_anio = moment(new Date()).format("YYYY-MM-DD").toString().substr(0, 4);
        this.iDocumento.nfm_estado = "A";
        this.iDocumento.nfm_observacion = "";
  ...
  */
    ...
    mensaje(_tipo: string, _mensaje: string) {
        this.llamarmodal = _tipo + "|Nomina Fin Mes|" + _mensaje + "|" + Funcion.Ramdon().toString();
    }
}
