...
*/
@Component({
    templateUrl: './pagodecimo13.component.html',
    styleUrls: ['./pagodecimo13.component.scss']
})
export class PagoDecimo13Component implements OnInit {
    /**
  ...
  */
   ...
   */
    router: Router;
    ban: number = 0;
    ColumnaDetalle: any[] = ['Identificacion', 'Nombre Empleado', 'Fecha Contrato','Periodo Decimo','Valor Decimo'];
    /**
...
*/
    cargaAnticipo() {
        this.spinner.show();
        this.mantenimientoService.getDecimos(globales.cia,"2").subscribe(data => {
            try {
...
*/
    ...
    calcularcuota() {
        ...
        this.spinner.show();
        this.mantenimientoService.generadecimo13(globales.cia, this.iDocumento.dec_anio).subscribe(data => {
            try {
                if (data.codError === "0") {
  ...
  */
    constructor(private spinner: NgxSpinnerService, _router: Router, private modalService: BsModalService, private fb: FormBuilder, private mantenimientoService: MantenimientoService, private injector: Injector) {
        ...
        if (!Funcion.ValidadAutorizado()) {
            ...
            if (!Funcion.ValidadPagina('Pago Decimo 13')) {
    ...
    delete() {
        this.spinner.show();
        this.iDocumento.dec_estado = "D";
        this.mantenimientoService.deletDecimo13(this.iDocumento).subscribe(data => {
            try {
                if (data.success) {
  ...
  */
    ...
    confirmarguardar() {
        if (this.isAccion === "I") {
            this.mensaje("4", "Esta seguro de generar el decimo 13 salario");
        } else if (this.isAccion === "M") {
            this.mensaje("4", "Esta seguro de modificar el decimo 13 salario");
        }
    ...
    updatePrestamo() {
        ...
        this.mantenimientoService.updateDecimo13(this.iDocumento).subscribe(data => {
            try {
                if (data.success) {
                    var mensaje: string = "";
    ...
    calculoquincena() {
        this.iDocumento.dec_valor_total = "";
        this.detallequincena.forEach((x) => {
            this.iDocumento.dec_valor_total = ((this.iDocumento.dec_valor_total === "") ? 0 : parseFloat(this.iDocumento.dec_valor_total)) + parseFloat(x.decimo_14);
        });
        return this.iDocumento.dec_valor_total;
    }
    insertPrestamo() {
        this.spinner.show();
        this.iDocumento.co_codigo = globales.cia;
        this.iDocumento.dec_estado = "A";
        this.iDocumento.us_codigo = Funcion.returdatosession("usuario").us_codigo;
        this.iDocumento.detalle = this.detallequincena;
        this.mantenimientoService.insertDecimo13(this.iDocumento).subscribe(data => {
            try {
                if (data.success) {
                    var mensaje: string = "";
                    mensaje = mensaje + data.msgError + "<br/>";
    ...
    validar() {
        ...
        if (this.detallequincena.length <= 0) {
            this.smensaje = "No se ha cargado el sueldo decimo 13...Por Favor Revisar"
            return;
        }
    ...
    modificarquincena() {
        if (this.iDocumento.cob_tipo === null) {
            ...
            this.mensaje("1", "No se puede modificar el decimo 13 sueldo ya esta contabilizada ...Por Favor Revisar")
            return;
        }
    }
    seteaParametro(data: any) {
        ...
        this.spinner.show();
        this.mantenimientoService.getseleDecimo13(data.co_codigo, data.dec_sec).subscribe(data => {
            try {
                if (data.codError === "0") {
                    this.iDocumento = data.root[0][0];
                    this.iDocumento.dec_fecha_registro = moment(data.dec_fecha_registro).format("YYYY-MM-DD").toString();

       
                    this.iDocumento.dec_anio = parseInt(this.iDocumento.dec_anio);
  ...
  */
    limpiarparametro() {
        this.iDocumento = {};
        this.iDocumento.dec_fecha_registro = moment(new Date()).format("YYYY-MM-DD").toString()
        this.iDocumento.dec_anio = moment(new Date()).format("YYYY-MM-DD").toString().substr(0, 4);
        this.iDocumento.dec_estado = "A";
        this.iDocumento.dec_observacion = "";
        this.iDocumento.dec_tipo = "2";
        this.iDocumento.dec_valor_total = "";
  ...
  */
    ...
    mensaje(_tipo: string, _mensaje: string) {
        this.llamarmodal = _tipo + "|Pago Decimo 13|" + _mensaje + "|" + Funcion.Ramdon().toString();
    }
}
