...
*/
@Component({
    templateUrl: './pagodecimo14.component.html',
    styleUrls: ['./pagodecimo14.component.scss']
})
export class PagoDecimo14Component implements OnInit {
    /**
...
*/
    cargaAnticipo() {
        this.spinner.show();
        this.mantenimientoService.getDecimos(globales.cia,"1").subscribe(data => {
            try {
...
*/
    ...
    calcularcuota() {
        ...
        this.spinner.show();
        this.mantenimientoService.generadecimo14(globales.cia, this.iDocumento.dec_anio).subscribe(data => {
            try {
                if (data.codError === "0") {
  ...
  */
    constructor(private spinner: NgxSpinnerService, _router: Router, private modalService: BsModalService, private fb: FormBuilder, private mantenimientoService: MantenimientoService, private injector: Injector) {
        ...
        if (!Funcion.ValidadAutorizado()) {
            ...
            if (!Funcion.ValidadPagina('Pago Decimo 14')) {
    ...
    delete() {
        ...
        this.mantenimientoService.deletDecimo14(this.iDocumento).subscribe(data => {
            try {
                if (data.success) {
  ...
  */
    ...
    confirmarguardar() {
        if (this.isAccion === "I") {
            this.mensaje("4", "Esta seguro de generar el decimo 14 salario");
        } else if (this.isAccion === "M") {
            this.mensaje("4", "Esta seguro de modificar el decimo 14 salario");
        }
    ...
    updatePrestamo() {
        ...
        this.mantenimientoService.updateDecimo14(this.iDocumento).subscribe(data => {
            try {
                if (data.success) {
                    var mensaje: string = "";
    ...
    insertPrestamo() {
        ...
        this.iDocumento.detalle = this.detallequincena;
        this.mantenimientoService.insertDecimo14(this.iDocumento).subscribe(data => {
            try {
                if (data.success) {
                    var mensaje: string = "";
                    mensaje = mensaje + data.msgError + "<br/>";
    ...
    validar() {
        ...
        if (this.detallequincena.length <= 0) {
            this.smensaje = "No se ha cargado el sueldo decimo 14...Por Favor Revisar"
            return;
        }
    ...
    modificarquincena() {
        if (this.iDocumento.cob_tipo === null) {
            ...
            this.mensaje("1", "No se puede modificar el sueldo decimo 14 ya esta contabilizada ...Por Favor Revisar")
            return;
        }
    }
    seteaParametro(data: any) {
        ...
        this.spinner.show();
        this.mantenimientoService.getseleDecimo14(data.co_codigo, data.dec_sec).subscribe(data => {
            try {
                if (data.codError === "0") {
  ...
  */
    ...
    mensaje(_tipo: string, _mensaje: string) {
        this.llamarmodal = _tipo + "|Pago Decimo 14|" + _mensaje + "|" + Funcion.Ramdon().toString();
    }
}
