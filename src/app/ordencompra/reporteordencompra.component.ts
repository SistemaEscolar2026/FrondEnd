...
@Component({
    ...
    styleUrls: ['./reporteordencompra.component.scss']
})
export class ReporteOrdenCompraComponent implements OnInit {
    /**
  ...
  */
    ...
    modalRef: any;
    llamarmodal: string = "";
    ColumnaDetalle: any[] = ['ORDEN COMPRA', 'PROVEEDOR', 'FECHA ORDEN COMPRA', 'TIPO ORDEN COMPRA', 'FORMA PAGO', 'OBSERVACION', 'TOTAL ORDEN COMPRA'];
 ...
 */
    ...
    buscarexcel() {
        this.spinner.show();
        this.boletoService.repoteOrdenCompra("EXC", globales.cia, this.iDocumento.bc_fecha_inicio_viaje, this.iDocumento.bc_fecha_fin_viaje, this.iDocumento.cl_codigo.toString(), globales.nomMaquina, Funcion.ReturnUsuario().us_login).subscribe((results) => {
            try {
                ...
                a.href = fileURL;
                a.download = "ReporteOrdenCompra.xlsx";
                a.click();
                this.spinner.hide();
    ...
    buscarpdf() {
        this.spinner.show();
        this.boletoService.repoteOrdenCompra("PDF", globales.cia, this.iDocumento.bc_fecha_inicio_viaje, this.iDocumento.bc_fecha_fin_viaje, this.iDocumento.cl_codigo.toString(), globales.nomMaquina, Funcion.ReturnUsuario().us_login).subscribe((results) => {
            try {
                try {
    ...
    buscarcuentaxcobrar() {
        this.spinner.show();
        this.boletoService.repoteOrdenCompraVisual("", globales.cia, this.iDocumento.bc_fecha_inicio_viaje, this.iDocumento.bc_fecha_fin_viaje, this.iDocumento.cl_codigo.toString()).subscribe(data => {
            try {
    ...
    totalColumna(col: number) {
        ...
        if (col === 1) {
            this.detallereporte.forEach((key) => {
                _cantidad = _cantidad + key.ord_total;
            });
        }
