...
*/
@Component({
    templateUrl: './reporteantquincena.component.html',
    styleUrls: ['./reporteantquincena.component.scss']
})
export class ReporteAntQuincenaComponent implements OnInit {
    /**
  ...
  */
    ...
    modalRef: any;
    llamarmodal: string = "";
    ColumnaDetalle: any[] = ['COD. EMPLEADO', 'NOMBRE EMPLEADO', 'FECHA DE ADELANTO QUINCENA', 'VALOR ADELANTO QUINCENA', 'OBSERVACION ADELANTO QUINCENA'];
 ...
 */
    ...
    cancelar() {
        ...
        this.iDocumento.empcodigo = 0;
        this.iDocumento.empnombres = "";

    }
    buscarexcel() {
        this.spinner.show();
        this.boletoService.repoteAdelanto("EXC", globales.cia, this.iDocumento.bc_fecha_inicio_viaje, this.iDocumento.bc_fecha_fin_viaje, (this.iDocumento.empcodigo === undefined) ? "0" : this.iDocumento.empcodigo.toString()).subscribe((results) => {
            try {
                ...
                a.href = fileURL;
                a.download = "ReporteAdelanto.xlsx";
                a.click();
                this.spinner.hide();
    ...
    buscarpdf() {
        this.spinner.show();
        this.boletoService.repoteAdelanto("PDF", globales.cia, this.iDocumento.bc_fecha_inicio_viaje, this.iDocumento.bc_fecha_fin_viaje, (this.iDocumento.empcodigo === undefined) ? "0" : this.iDocumento.empcodigo.toString()).subscribe((results) => {
            try {
                try {
    ...
    buscaradelanto() {
        this.spinner.show();
        this.boletoService.repoteAdelantoVisual("", globales.cia, this.iDocumento.bc_fecha_inicio_viaje, this.iDocumento.bc_fecha_fin_viaje, (this.iDocumento.empcodigo === undefined) ? "0" : this.iDocumento.empcodigo.toString()).subscribe(data => {
            try {
...
*/
  ...
  */
    ...
    cierreEmpleado(event: any) {
        if (event != "") {
            this.cierreModalcliente(true);
            //codigo
            var _dato: any = JSON.parse(event);
            this.seteaCliente(_dato);
        }
    }

    seteaCliente(_datos: any) {
        this.iDocumento.empcodigo = _datos.empsec;
        this.iDocumento.empnombres = _datos.empnombres + " " + _datos.empapellidopaterno + " " + _datos.empapellidomaterno;
...
*/
  ...
  */
    ...
    mensaje(_tipo: string, _mensaje: string) {
        this.llamarmodal = _tipo + "|Reporte de Adelanto Quincena|" + _mensaje + "|" + Funcion.Ramdon().toString();
    }
