...
*/
@Component({
    templateUrl: './reportecontrato.component.html',
    styleUrls: ['./reportecontrato.component.scss']
})
export class ReporteContratoComponent implements OnInit {
  ...
  */
    ...
    modalRef: any;
    llamarmodal: string = "";
    ColumnaDetalle: any[] = ['ENTIDAD', 'CONTRATO', 'VALOR DE CONTRATO', 'FECHA DE EMISION', 'OPERADOR', 'N. BOLETO', 'IDENTIFICACION DEL PASAJERO', 'NOMBRE DEL PASAJERO', 'RUTA', 'FECHA DE IDA', 'FECHA DE RETORNO', 'TOTAL BOLETO + SERVICIO', 'NETO', 'GANANCIA', 'NAC/INT', '# RESERVA'];
 ...
 */
    ...
    buscarexcel() {
        this.spinner.show();
        this.boletoService.repoteContratos("EXC", globales.cia, this.iDocumento.bc_fecha_inicio_viaje, this.iDocumento.bc_fecha_fin_viaje, this.iDocumento.cl_codigo.toString(), globales.nomMaquina, Funcion.ReturnUsuario().us_login).subscribe((results) => {
            try {
    ...
    buscarpdf() {
        this.spinner.show();
        this.boletoService.repoteContratos("PDF", globales.cia, this.iDocumento.bc_fecha_inicio_viaje, this.iDocumento.bc_fecha_fin_viaje, this.iDocumento.cl_codigo.toString(), globales.nomMaquina, Funcion.ReturnUsuario().us_login).subscribe((results) => {
            try {
                try {
    ...
    buscarboleto() {
        this.spinner.show();
        this.boletoService.repoteContratoVisual("", globales.cia, this.iDocumento.bc_fecha_inicio_viaje, this.iDocumento.bc_fecha_fin_viaje, this.iDocumento.cl_codigo.toString()).subscribe(data => {
            try {
