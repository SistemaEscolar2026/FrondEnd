...
*/
@Component({
    templateUrl: './reportecuentaxcobrar.component.html',
    styleUrls: ['./reportecuentaxcobrar.component.scss']
})
export class ReporteCuentaxCobrarComponent implements OnInit {
    /**
  ...
  */
    ...
    modalRef: any;
    llamarmodal: string = "";
    ColumnaDetalle: any[] = ['FACTURA', 'FECHA DE FACTURA', 'ENTIDAD',  'FECHA DE FACTURA', 'FECHA DE VENCIMIENTO', 'MENOR DE 30 DIAS', 'MAYOR 30 Y MENOR DE 45 DIAS', 'MAYOR 45 Y MENOR DE 60 DIAS', 'MAYOR 60 DIAS'];
 ...
 */
    ...
    buscarexcel() {
        this.spinner.show();
        this.boletoService.repoteCuentaxCobrar("EXC", globales.cia, this.iDocumento.bc_fecha_inicio_viaje, this.iDocumento.bc_fecha_fin_viaje, this.iDocumento.cl_codigo.toString(), globales.nomMaquina, Funcion.ReturnUsuario().us_login).subscribe((results) => {
            try {
                ...
                a.href = fileURL;
                a.download = "ReporteCuentaxCobrar.xlsx";
                a.click();
                this.spinner.hide();
    ...
    buscarpdf() {
        this.spinner.show();
        this.boletoService.repoteCuentaxCobrar("PDF", globales.cia, this.iDocumento.bc_fecha_inicio_viaje, this.iDocumento.bc_fecha_fin_viaje, this.iDocumento.cl_codigo.toString(), globales.nomMaquina, Funcion.ReturnUsuario().us_login).subscribe((results) => {
            try {
                try {
    ...
    buscarcuentaxcobrar() {
        this.spinner.show();
        this.boletoService.repoteCuentaxCobrarVisual("", globales.cia, this.iDocumento.bc_fecha_inicio_viaje, this.iDocumento.bc_fecha_fin_viaje, this.iDocumento.cl_codigo.toString()).subscribe(data => {
            try {
    ...
    totalColumna(col: number) {
        ...
        if (col === 1) {
            this.detallereporte.forEach((key) => {
                _cantidad = _cantidad + key.meno30dias;
            });
        }
        if (col === 2) {
            this.detallereporte.forEach((key) => {
                _cantidad = _cantidad + key.manyo30ymenor45dias;
            });
        }
        if (col === 3) {
            this.detallereporte.forEach((key) => {
                _cantidad = _cantidad + key.manyo45ymenor60dias;
            });
        }
        if (col === 4) {
            this.detallereporte.forEach((key) => {
                _cantidad = _cantidad + key.manyo60dias;
            });
        }
