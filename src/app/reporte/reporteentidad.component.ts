...
*/
@Component({
    templateUrl: './reporteentidad.component.html',
    styleUrls: ['./reporteentidad.component.scss']
})
export class ReporteEntidadComponent implements OnInit {
    /**
  ...
  */
    ...
    modalRef: any;
    llamarmodal: string = "";
    ColumnaDetalle: any[] = ['TIPO CLIENTE', 'IDENTIFICACION CLIENTE', 'NOMBRE CLIENTE', 'REPRESENTANTE LEGAL', 'TELEFONO', 'DIRECCION', 'CORREO', 'FECHA CREACION', 'ESTADO'];
 ...
 */
    ...
    buscarexcel() {
        this.spinner.show();
        this.boletoService.reporteentidadvisualpdf("EXC", globales.cia, this.iDocumento.bc_fecha_inicio_viaje, this.iDocumento.bc_fecha_fin_viaje, this.iDocumento.cl_codigo.toString()).subscribe((results) => {
            try {
                ...
                a.href = fileURL;
                a.download = "ReporteEntidad.xlsx";
                a.click();
                this.spinner.hide();
    ...
    buscarpdf() {
        this.spinner.show();
        this.boletoService.reporteentidadvisualpdf("PDF", globales.cia, this.iDocumento.bc_fecha_inicio_viaje, this.iDocumento.bc_fecha_fin_viaje, this.iDocumento.cl_codigo.toString()).subscribe((results) => {
            try {
                try {
    ...
    buscarboleto() {
        this.spinner.show();
        this.boletoService.reporteentidadvisual("", globales.cia, this.iDocumento.bc_fecha_inicio_viaje, this.iDocumento.bc_fecha_fin_viaje, this.iDocumento.cl_codigo.toString()).subscribe(data => {
            try {
  ...
  */
    constructor(private spinner: NgxSpinnerService, _router: Router, private modalService: BsModalService, private fb: FormBuilder, private mantenimientoService: MantenimientoService, private boletoService: BoletoService, private injector: Injector) {
        ...
        if (!Funcion.ValidadAutorizado()) {
            ...
            if (!Funcion.ValidadPagina('Reporte Entidad')) {
...
*/
  ...
  */
    ...
    mensaje(_tipo: string, _mensaje: string) {
        this.llamarmodal = _tipo + "|Ingreso de Entidad|" + _mensaje + "|" + Funcion.Ramdon().toString();
    }
