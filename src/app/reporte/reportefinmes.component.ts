...
*/
@Component({
    templateUrl: './reportefinmes.component.html',
    styleUrls: ['./reportefinmes.component.scss']
})
export class ReporteFinMesComponent implements OnInit {
    /**
   * DEFINICION DE VARIABLE DE MODAL DE BUSQUEDA DE VENDEDOR
   */
    @ViewChild('modalbusquedaentidad') modalbusquedaentidad: any;

    ColumnaDetalle: any[] = ['Cedula', 'Nombre y Apellido', 'Sueldo Fijo', 'Dias Trab.', 'Sueldo', 'F. Reserva', 'Decimo Men.', 'TOTAL INGRESO', 'Aporte 9.45%', 'Ant. Quincena', 'Prestamo Quiro.', 'Pres. A Empresa', 'Dsctos  Varios', 'TOTAL EGRESO', 'NETO A PAGAR'];
    listmeses: any[] = [];
 ...
 */
    ...
    buscarexcel() {
        this.spinner.show();
        this.boletoService.repoteFinMesPDF("EXC", globales.cia, this.nfm_anio, this.nfm_mes.toString(), globales.nomMaquina, Funcion.ReturnUsuario().us_login).subscribe((results) => {
            try {
                ...
                a.href = fileURL;
                a.download = "ReporteNominaFinMes.xlsx";
                a.click();
                this.spinner.hide();
    ...
    buscarpdf() {
        this.spinner.show();
        this.boletoService.repoteFinMesPDF("PDF", globales.cia, this.nfm_anio, this.nfm_mes.toString(), globales.nomMaquina, Funcion.ReturnUsuario().us_login).subscribe((results) => {
            try {
                try {
    ...
    buscar() {
        this.spinner.show();
        this.boletoService.repoteFinMesVisual("", globales.cia, this.nfm_anio, this.nfm_mes.toString()).subscribe(data => {
            try {
