...
*/
@Component({
    templateUrl: './reporteganacia.component.html',
    styleUrls: ['./reporteganacia.component.scss']
})
export class ReporteGananciaComponent implements OnInit {
  ...
  */
    ...
    modalRef: any;
    llamarmodal: string = "";
    ColumnaDetalle: any[] = ['CLIENTE', 'FACTURA', 'COSTO', 'UTILIDAD', '%'];
 ...
 */
    ...
    cancelar() {
        ...
        this.iDocumento.bc_evalua = true;
    }
    buscarexcel() {
        let _lis = ""
        this.codentidad.forEach((key) => {
            _lis = _lis + key;
        });
        this.spinner.show();
        this.boletoService.repoteGanancias("EXC", globales.cia, this.iDocumento.bc_fecha_inicio_viaje, this.iDocumento.bc_fecha_fin_viaje, _lis, globales.nomMaquina, Funcion.ReturnUsuario().us_login, this.iDocumento.bc_evalua).subscribe((results) => {
            try {
                ...
                a.href = fileURL;
                a.download = "ReporteUtilidades.xlsx";
                a.click();
                this.spinner.hide();
    ...
    buscaradelanto() {
        ...
        this.spinner.show();
        this.boletoService.repoteGanancia("", globales.cia, this.iDocumento.bc_fecha_inicio_viaje, this.iDocumento.bc_fecha_fin_viaje, _lis, this.iDocumento.bc_evalua).subscribe(data => {
            try {
                this.detallereporte = data.root[0];
                var _dato: any[] = data.root[1];

                _dato.forEach((x) => {
                    this.detallereporte.push({
                        cl_codigo: x.cod,
                        cl_nombre: x.gasto,
                        factura: x.valor,
                        costo: x.uneta,
