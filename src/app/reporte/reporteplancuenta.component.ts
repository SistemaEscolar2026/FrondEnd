...
*/
@Component({
    templateUrl: './reporteplancuenta.component.html',
    styleUrls: ['./reporteplancuenta.component.scss']
})
export class ReportePlanCuentaComponent implements OnInit {
    /**
  ...
  */
    ...
    modalRef: any;
    llamarmodal: string = "";
    ColumnaDetalle: any[] = ['GRUPO', 'SUBGRUPO', 'CODIGO CUENTA', 'DESCRIPCION', 'CLASE CUENTA', 'NATURALEZA', 'AUXILIAR'];
    /**
 ...
 */
    ...
    cargaGrupo() {
        ...
        this.mantenimientoService.manGrupo(globales.cia).subscribe(data => {
            try {
                this.listgrupocontable = [];
                this.listgrupocontable.push({
                    con_codigo: 0,
                    con_descripcion: "TODOS"
                });
                var dato: any[] = data.root[0];
                dato.forEach((k) => {
                    this.listgrupocontable.push({
                        con_codigo: k.con_codigo,
                        con_descripcion: k.con_descripcion
                    });
                });
                this.codgrupocontable = "0";
                this.spinner.hide();
            } catch (e) {
                this.spinner.hide();
            }
    ...
    cargasubgruposelect(datospaso: string) {
        ...
        var datos: any[] = this.listsubgrupo.filter(k => k.con_codigo === parseInt(this.codgrupocontable));

        this.listsubgrupocontable.push({
            cos_codigo: 0,
            con_codigo: "",
            cos_descripcion: "TODOS"
        });
        datos.forEach((k) => {
            this.listsubgrupocontable.push({
    ...
    buscarexcel() {
        this.spinner.show();
        this.boletoService.repotePlanCuentarpt("EXC", globales.cia, this.codgrupocontable, this.codsubgrupo, globales.nomMaquina, Funcion.ReturnUsuario().us_login).subscribe((results) => {
            try {
                ...
                a.href = fileURL;
                a.download = "ReportePlanCuenta.xlsx";
                a.click();
                this.spinner.hide();
    ...
    buscarpdf() {
        this.spinner.show();
        this.boletoService.repotePlanCuentarpt("PDF", globales.cia, this.codgrupocontable, this.codsubgrupo, globales.nomMaquina, Funcion.ReturnUsuario().us_login).subscribe((results) => {
            try {
                try {
    ...
    buscarPlanCuenta() {
        this.spinner.show();
        this.boletoService.repotePlanCuenta("", globales.cia, this.codgrupocontable, this.codsubgrupo).subscribe(data => {
            try {
  ...
  */
    constructor(private spinner: NgxSpinnerService, _router: Router, private modalService: BsModalService, private fb: FormBuilder, private mantenimientoService: MantenimientoService, private boletoService: BoletoService, private injector: Injector) {
        ...
        if (!Funcion.ValidadAutorizado()) {
            ...
            if (!Funcion.ValidadPagina('Reporte Comprobante Contable')) {
