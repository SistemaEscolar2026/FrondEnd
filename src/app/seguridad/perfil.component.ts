...
export class PerfiComponent implements OnInit {
    /**
  * DEFINICION DE VARIABLE DE MODAL DE BUSQUEDA DE VENDEDOR
  */
    @ViewChild('modalbusquedaperfil') modalbusquedaperfil: any;
    /**
...
*/
    cargaPais() {
        ...
        this.mantenimientoService.manPerfil(globales.cia).subscribe(data => {
            try {
                this.listadoperfil = data.root[0];
                this.openModal(this.modalbusquedaperfil);
                this.spinner.hide();
            } catch (e) {
                this.spinner.hide();
            }
  ...
  */
    deletepais() {
        this.spinner.show();
        this.model.sp_sec = this.paisfrom.controls['codigo'].value.toString().toUpperCase();
        this.model.sp_estado = "D";
        this.mantenimientoService.deletePerfil(this.model).subscribe(data => {
            try {
                if (data.success) {
  ...
  */
    insertpais() {
        this.spinner.show();
        this.model.sp_sec = this.paisfrom.controls['codigo'].value.toString().toUpperCase();
        this.model.sp_nombre = this.paisfrom.controls['descripcion'].value.toString().toUpperCase();
        this.model.sp_estado = this.paisfrom.controls['estado'].value.toString().toUpperCase();
        this.mantenimientoService.insertPerfil(this.model).subscribe(data => {
            try {
                if (data.success) {
                    this.notifier.showok(data.msgError, "")
                    this.limpiabotones(1);
  ...
  */
    updatepais() {
        ...
        this.model.sp_nombre = this.paisfrom.controls['descripcion'].value.toString().toUpperCase();
        this.model.sp_estado = this.paisfrom.controls['estado'].value.toString().toUpperCase();
        this.mantenimientoService.updatePerfil(this.model).subscribe(data => {
            try {
                if (data.success) {
                    this.notifier.showok(data.msgError, "")
                    this.limpiabotones(1);
  ...
  */
    seteaPais(data: any) {
        ...
        this.paisfrom.controls['codigo'].setValue(data.sp_sec);
        this.paisfrom.controls['descripcion'].setValue(data.sp_nombre);
        this.paisfrom.controls['estado'].setValue(data.sp_estado);
