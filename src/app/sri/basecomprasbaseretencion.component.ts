...
*/
@Component({
    selector: 'app-basecomprasbaseretencion',
    templateUrl: './basecomprasbaseretencion.component.html',
    styleUrls: ['./basecomprasbaseretencion.component.scss']
})
export class BaseComprasBaseRetencionComponent {
    ...
    constructor(private spinner: NgxSpinnerService, _router: Router, private funcionesService: FuncionesService, private modalService: BsModalService, private bancosService: BancosService, private mantenimientoService: MantenimientoService, private sriService: SriService) {
        ...
        if (!Funcion.ValidadAutorizado()) {
            ...
            if (!Funcion.ValidadPagina('Comparativo de Base Compras y Base Retenciones por Proveedor')) {
    ...
    generaReporte() {
        ...
        if (this.rb_excel === "N") {
            iNombreRporte = "ComparativoBaseCxpRetencion";
        }
        else {
            iNombreRporte = "ComparativoBaseCxpRetencionCol";
        }


        this.openReport = "Comparativo de Base Compras y Base Retenciones por Proveedor|" + environment.reportServerSRI + iNombreRporte + "|" + JSON.stringify(parametro) + "|" + Funcion.Ramdon();
  ...
  */
    modalmensaje(tipo: string, mensaje: string) {
        this.llamarmodal = tipo + "|Comparativo de Base Compras y Base Retenciones por Proveedor|" + mensaje + "|" + Funcion.Ramdon();
    }
