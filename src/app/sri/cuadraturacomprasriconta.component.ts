...
*/
@Component({
    selector: 'app-cuadraturacomprasriconta',
    templateUrl: './cuadraturacomprasriconta.component.html',
    styleUrls: ['./cuadraturacomprasriconta.component.scss']
})
export class CuadraturaComprasSriContabilidadComponent {
    ...
    constructor(private spinner: NgxSpinnerService, _router: Router, private funcionesService: FuncionesService, private modalService: BsModalService, private bancosService: BancosService, private mantenimientoService: MantenimientoService,  private sriService: SriService) {
        ...
        if (!Funcion.ValidadAutorizado()) {
            ...
            if (!Funcion.ValidadPagina('Cuadratura de Retenciones de Iva Compra - Sri - Contabilidad')) {
    ...
    generaReporte() {
        ...
        if (this.rb_excel === "N") {
            iNombreRporte = "CuadraturaRetencionesIvaComprasSriContabilidad";
        }
        else {
            iNombreRporte = "CuadraturaRetencionesIvaComprasSriContabilidadCol";
        }


        this.openReport = "Cuadratura de Retenciones de Iva Compras - Sri - Contabilidad|" + environment.reportServerSRI + iNombreRporte + "|" + JSON.stringify(parametro) + "|" + Funcion.Ramdon();
  ...
  */
    modalmensaje(tipo: string, mensaje: string) {
        this.llamarmodal = tipo + "|Cuadratura de Retenciones de Iva Compras - Sri - Contabilidad|" + mensaje + "|" + Funcion.Ramdon();
    }
