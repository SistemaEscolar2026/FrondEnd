...
*/
@Component({
    templateUrl: './cuadraturaretcarconta.component.html',
    styleUrls: ['./cuadraturaretcarconta.component.scss']
})
export class CuadraturaRetencionesCarteraContabilidadComponent {
    ...
    constructor(private spinner: NgxSpinnerService, _router: Router, private funcionesService: FuncionesService, private modalService: BsModalService, private bancosService: BancosService, private mantenimientoService: MantenimientoService, private sriService: SriService) {
        ...
        if (!Funcion.ValidadAutorizado()) {
            ...
            if (!Funcion.ValidadPagina('Cudratura De Retenciones Cartera - Contabilidad')) {
    ...
    generaReporte() {
        ...
        if (this.rb_excel === "N") {
            iNombreRporte = "CuadraturaRetencionesCarteraContabilidad";
        }
        else {
            iNombreRporte = "CuadraturaRetencionesCarteraContabilidadCol";
        }


        this.openReport = "Cuadratura de Retenciones Cartera - Contabilidad|" + environment.reportServerSRI + iNombreRporte + "|" + JSON.stringify(parametro) + "|" + Funcion.Ramdon();
  ...
  */
    modalmensaje(tipo: string, mensaje: string) {
        this.llamarmodal = tipo + "|Cuadratura de Retenciones Cartera - Contabilidad|" + mensaje + "|" + Funcion.Ramdon();
    }
