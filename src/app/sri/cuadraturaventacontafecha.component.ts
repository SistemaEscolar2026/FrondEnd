...
*/
@Component({
    selector: 'app-cuadraturaventacontafecha',
    templateUrl: './cuadraturaventacontafecha.component.html',
    styleUrls: ['./cuadraturaventacontafecha.component.scss']
})
export class CuadraturaVentaContaFechaComponent {
    ...
    constructor(private spinner: NgxSpinnerService, _router: Router, private funcionesService: FuncionesService, private modalService: BsModalService, private bancosService: BancosService, private mantenimientoService: MantenimientoService,  private sriService: SriService) {
        ...
        if (!Funcion.ValidadAutorizado()) {
            ...
            if (!Funcion.ValidadPagina('Cuadratura de Iva Ventas Contabilidad por Fecha')) {
    ...
    generaReporte() {
        ...
        if (this.rb_excel === "N") {
            iNombreRporte = "CuadraturaIvaVentaContabDet";
        }
        else {
            iNombreRporte = "CuadraturaIvaVentaContabDetCol";
        }


        this.openReport = "Cuadratura de Iva Ventas-Contabilidad por Fecha|" + environment.reportServerSRI + iNombreRporte + "|" + JSON.stringify(parametro) + "|" + Funcion.Ramdon();
  ...
  */
    modalmensaje(tipo: string, mensaje: string) {
        this.llamarmodal = tipo + "|Cuadratura de Iva Ventas-Contabilidad por Fecha|" + mensaje + "|" + Funcion.Ramdon();
    }
