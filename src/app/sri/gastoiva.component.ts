...
*/
@Component({
    selector: 'app-gastoiva',
    templateUrl: './gastoiva.component.html',
    styleUrls: ['./gastoiva.component.scss']
})
export class GastoIvaComponent {
    ...
    constructor(private spinner: NgxSpinnerService, _router: Router, private funcionesService: FuncionesService, private modalService: BsModalService, private bancosService: BancosService, private mantenimientoService: MantenimientoService, private sriService: SriService) {
        ...
        if (!Funcion.ValidadAutorizado()) {
            ...
            if (!Funcion.ValidadPagina('Reporte de Gasto IVA')) {
    ...
    generaReporte() {
        ...
        if (this.rb_excel === "N") {
            iNombreRporte = "DeterminacionGastoIvaSuc";
        }
        else {
            iNombreRporte = "DeterminacionGastoIvaSucCol";
        }


        this.openReport = "Reporte de Gasto IVA|" + environment.reportServerSRI + iNombreRporte + "|" + JSON.stringify(parametro) + "|" + Funcion.Ramdon();
  ...
  */
    modalmensaje(tipo: string, mensaje: string) {
        this.llamarmodal = tipo + "|Reporte de Gasto IVA|" + mensaje + "|" + Funcion.Ramdon();
    }
